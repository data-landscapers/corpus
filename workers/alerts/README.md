...
**Bindings, and the names are load-bearing.** KV `ALERTS` → namespace `alerts`; secrets `BUTTONDOWN_API_KEY` and `TURNSTILE_SECRET`; variables `SITE`, `MAIN_SITE` and `SEND_MODE`. One cron trigger, `0 7 * * MON` — the day by name, because Cloudflare counts `1` as Sunday. `documentation/catalogue-alerts.md` Part 1 C is the deployment, step by step.
