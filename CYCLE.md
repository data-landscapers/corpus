...
**What the quiet answer still cannot tell apart is *OSINT has not run* from *the mirror has not carried what it ran*, and that is now measured a step later** *(2026-08-22, `notes-for-corpus.md` note 5)*. Both read here as `nothing new`, and both mean the same thing to a build — the base has not moved — so the trigger is right either way and there is nothing for it to do differently. What the second case additionally means is that a build fired by hand would compile from a copy that is *older than OSINT thinks it is*, which no exit code here would say. `scripts/lint-osint-freshness.py` says it, at `BUILD.md` stage 0: three clocks off the mirror, the newest reported, a 72-hour ceiling, and a regression test against the close already built. It reports and does not gate, for the same reason this trigger does not fire on base movement — a cycle is run by hand, so a quiet stretch is a schedule rather than a defect.
