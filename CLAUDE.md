...
**CC reads `C:\OSINT` and never writes to it, without exception.** No edits, no new files, no moves, no deletes, no git operations, no running of its procedure files anywhere else in the tree — including `log.md`, the review queues and every process file. **Writing is what the boundary forbids; reading is not restricted** *(Bill, 2026-09-25)* — the prohibition on writing has not narrowed, and never does.

**What is bounded is attention, not access** *(Bill, 2026-09-25)*. CC may read anything in the mirror that serves a Corpus output — the evidence in `raw/`, `wiki/` and `lookups/`, the documents in `budget-archive/`, the mirror's `cycle-manifest.json` and its git metadata as a staleness clock. What it does not do is **watch OSINT**: Corpus once ran dedicated linters over OSINT's internal logs, parsed `ingested_log.md` forensically and carried incident histories of OSINT's defects in docstrings, and that time was wasted. So **no Corpus script reads OSINT's process material** — `logs/`, `reviews/`, `index/`, `new/`, `sweep/` or its process files — and `scripts/lint-interface.py` counts it, over the workroot junction list and every OSINT path a script builds; its exception list is empty and a stale exception fails. **An observation about OSINT that does not change a Corpus output is not logged, not noted and not mentioned** — the same test the note template applies. The rule is symmetrical: OSINT reads Corpus's data through the share and does not police Corpus's `logs/`, `documentation/` or runbooks.
...
## CC sends commits to OSINT's repository; it never makes one on `master`

**The folder is not the repository, and only the folder is forbidden** *(Bill, 2026-09-17, strategic review 4 R1)*. `C:\OSINT` is a mirror and CC never writes a byte to it — that rule is unchanged and absolute. What CC may do is prepare a change to OSINT's *repository* and hand it over: `scripts/osint-patch.py prepare` clones the mirror into `C:\osint-work` (outside both repositories), CC edits the clone, and `cut` commits it and writes a `git format-patch` series to `prepared/job-NN/` on the share with the base commit named beside it. OSINT runs its own containment check, applies with `git am --keep-cr -3`, lints and commits. **`master` has one writer and it is OSINT**; a conflict means `master` wins and CC re-cuts.

**A patch carries `scripts/`, `lookups/` and the three wiki index pages, and nothing else.** Not `raw/`: frontmatter changes travel as a script plus its input, because a patch is cut against a mirror that ingest rewrites nightly and across thousands of files a conflict is certain. Not wiki prose, which is where Phase B writes. Not OSINT's `logs/`, `reviews/` or process files, which CC does not read either. `scripts/lint-interface.py` holds the clone to that set and fails if `osint-patch.py`'s own allowed set is widened; `osint-patch.py` refuses to cut a straying series in the first place. The enforcing guard is OSINT's, on the side that can verify it.

## Pace — no freeze

**Corpus is not feature-frozen.** The freeze of 2026-08-28 was retired early by Bill on 2026-09-14: Corpus has no quota problem, and a live site has to answer feedback and feature requests quickly. Bill sets the balance between stability and moving; build what he asks for when he asks. Quota is watched on OSINT's side, where work other than sweeps goes through him.

## The exchange

**Everything CC produces goes in Corpus**, including anything about OSINT — except a document both sides work from, which lives in the share's `documentation/` *(Bill, 2026-09-23)*. If a change is needed *in* OSINT — a corrected path, a stale statement, a note for the queues — CC writes it as a numbered note in **`C:\corpus-osint-xfer\notes-for-osint.md`** and tells Bill; it does not make the change. Bill actions it in an OSINT session and strikes the note.

**`C:\corpus-osint-xfer\` holds the channel files, and delivery is Bill's call.** `notes-for-osint.md` and `notes-for-corpus.md` with an archive apiece (`…-resolved.md`), OSINT's housekeeping register `housekeeping-jobs.md` and its archive, the acquisition feed `africa-acquire.csv` and its far end `acquire-done.csv`, and **`new-queue/`, which OSINT's cycle pulls itself** *(Bill, 2026-09-17, review 4 R8 — the hand-carry of 2026-08-20 retires)*: a batch is a top-level folder, it is delivered when it carries a `READY` file, and CC **writes `READY` last** so a folder still being written is never pulled half-done. Delivery is not admission — the night's ingest adjudicates a pulled item like any other — and the pull's deletions here are CC's to commit on its next visit. Also **`prepared/`, work handed over ready to run**: a patch series cut by `scripts/osint-patch.py`, or a script with its input, which OSINT checks and applies itself. The share's own `README.md` is the canonical description of all of it, for both sides.

**The share's `README.md` is where the channel conventions live, and a channel file carries a pointer to it rather than a copy.** `scripts/lint-preambles.py` holds each preamble to 250 words and fails on a rule restated away from home — a rule stated twice is one that will eventually disagree with itself. It reports rather than fails on `housekeeping-jobs.md` and `messages-from-bill.md`, which are OSINT's and Bill's; OSINT asserts the same over its own files from `LINT.md` #24. Run it, `scripts/lint-notes.py` and `scripts/lint-prepared.py` whenever the share is edited — the last says what `prepared/` is holding that the job it was cut for has already closed, which is CC's to delete in the commit recording that close.

**What OSINT's cycle reads on the share, it reads because its own process files tell it to** *(amended 2026-09-17, review 4 R8)*: the notes queue at the night's first act, `new-queue/` folders carrying `READY`, the housekeeping register, the acquisition feed and anything waiting in `prepared/` for the job it is working. **Everything else still depends on Bill.** A note is not acted on until he says so, nothing else is picked up by being left somewhere, and CC never tells OSINT to read a `C:\CORPUS` path — a session there cannot see Corpus at all, the same isolation that keeps Corpus out of OSINT. Corpus's own `documentation/` remains Corpus-side and hand-carried. (The retired request feed `logs/requests-for-osint.csv` holds a header row and nothing else, and is left in place.)
