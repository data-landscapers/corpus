---
title: DR Congo — digital transformation and data governance status report
compiled: 2026-08-06
place: COD
ledger_rows: 77
not_held: 10
---

# DR Congo: status report

*Compiled 2026-08-06 from the Data Landscapers source base, from `outputs/reports/COD/ledger.csv` (77 systems and instruments, 10 of them ***Not held***). Each section opens with its ledger; specific events are covered in the monthly updates. Figures are dated because most are time-varying.*

**Status values.** *Implemented* — in operation or in force. *Piloting* — running with a limited user group or in a controlled environment. *In development* — build or drafting under way, not yet operating. *Planned* — announced or provided for, no build or draft on record. *Discontinued* — closed or superseded. ***Not held*** — the base carries no reliable statement of status; these are the gaps to fill and are listed again at the end.

## Summary of position

<!-- narrative: summary -->
DR Congo's digital appropriations can be read directly from an enacted budget. The 2026 finance law gives the digital economy section [FC 81,698,223,741 of domestic credit and posts and telecommunications FC 53,840,917,417](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf). Against that, the FY2025 chain closed with [FC 31,358,784,468 paid, 51.7% of the revised credit](https://www.budget.gouv.cd/wp-content/uploads/budget2025/budget2025_rect/lf_2025_depenses.pdf), and nothing had been published for the 2026 exercise seven months into the year.

The framework law is in force and its supervisory institution is not. The [Code du numérique was promulgated on 13 March 2023](https://actualite.cd/2026/04/21/rdc-un-nouvel-ouvrage-sur-la-protection-des-donnees-personnelles-porte-sur-les-fonts) with a third book governing personal data; the [authority required by its article 262 has still not been created](https://www.wearetech.africa/fr/fils/actualites/tech/numerique-la-rdc-met-en-place-un-reseau-de-responsables-de-la-protection-des-donnees), and no decree defining its organisation was found as at March 2026. A national identity platform meanwhile runs under a twenty-year exclusive concession, [launched on 13 June 2026 and valued at US$97.1m](https://www.biometricupdate.com/202606/drc-launches-rdc-pass-digital-id-platform-under-20-year-ppp-with-trident), with no enrolment figure published.

Physical build is measurable and behind. [7,000 km of the transformation project's fibre is deployed against a target of more than 18,500 km](https://deskeco.com/2026/08/03/transformation-numerique-en-rdc-encore-11500-km-de-fibre-optique-deployer-pour-atteindre-un-total-de), tied to connecting 145 territories by 2030. Ten of the rows below are ***Not held***.
<!-- /narrative -->

## Infrastructure

| System or instrument | Status | As at |
|---|---|---|
| 5G public consultation | [In development](https://actualite.cd/2025/03/18/larptc-lance-une-consultation-publique-sur-lintroduction-de-la-5g-en-rdc-communique) | The regulator opened a public consultation on introducing 5G |
| Africa Congo Internet Exchange | [Implemented](https://itweb.africa/article/distributed-exchange-boosts-drc-connectivity/4r1ly7R9GkLvpmda) | A second presence added at a Kinshasa carrier-neutral facility, making it the country's first distributed exchange |
| Child online-safety service | [Implemented](https://actualite.cd/2026/07/17/rdc-africell-lance-kidzonet-pour-renforcer-la-protection-des-enfants-sur-internet) | An operator launched the service at a Kinshasa press conference |
| Goma-Mombasa fibre route | [Implemented](https://itweb.africa/article/paratus-lights-up-new-east-africa-fibre-highway-linking-goma-to-mombasa/wbrpOqg2aJQMDLZn) | The route reported lit, linking Goma to Mombasa via Kampala |
| Kinshasa carrier-neutral data centre | [Implemented](https://techafricanews.com/2025/02/03/oadc-texaf-kinshasa-achieves-iso-and-pci-dss-certifications-strengthening-drcs-digital-future/) | Certified to information-security, continuity and payment-card standards |
| National cybersecurity agency | [Planned](https://acp.cd/economie/numerique-le-gouvernement-veut-une-chaine-de-responsabilite-visible-en-cybersecurite/) | Named as one of five bodies whose roles in the incident-notification chain must be clarified, explicitly without creating new institutions |
| National data centre appropriation | [Planned](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf) | The enacted 2026 finance law carries FC 26,756,546,346 for national data-centre acquisition |
| National digital transformation project fibre backbone | [In development](https://deskeco.com/2026/08/03/transformation-numerique-en-rdc-encore-11500-km-de-fibre-optique-deployer-pour-atteindre-un-total-de) | Deployment reported to the 95th ordinary Council meeting 2026-08-03 |
| Raxio Kinshasa data centre | [Implemented](https://actualite.cd/2025/10/31/infrastructures-la-rdc-franchit-un-cap-decisif-dans-sa-transformation-numerique-grace) | A contractor account of the delivered facility giving its build specification |
| Rural solar base-station joint venture | [In development](https://www.connectingafrica.com/connectivity/drc-connectivity-improves-but-some-usage-power-supply-gaps-linger) | The commitment restated as 2,000 new solar-powered base stations by 2031 |
| Sovereign government cloud | [Planned](https://actu7.cd/2026/07/29/augustin-kibassa-maliba-lance-le-chantier-dun-cadre-strategique-pour-accelerer-la-transformation-digitale-de-la-rdc/) | Named by the digital economy minister among the projects to be ordered and made executable |
| Sovereign satellite programme | [Planned](https://actualite.cd/2025/08/31/la-rdc-se-lance-dans-un-projet-dacquisition-dun-satellite-cout-estime-400-millions-de) | The President received the promoter's representative, who said a bank had mobilised the estimated US$400m |
| State telecommunications and postal operator | [Implemented, in structural crisis](https://rdc-monde.com/scpt-ce-que-lon-ne-dit-pas-sur-les-efforts-de-redressement-en-cours/) | A directorate-general communiqué on the recovery setting out finances against staff grievances |
| Energy provision for digital infrastructure | ***Not held*** | — |
| Functioning national incident-response capability | ***Not held*** | — |
| National technical-capacity base | ***Not held*** | — |

<!-- narrative: infrastructure -->
The backbone is the one infrastructure object with a delivered figure and a target. [7,000 km is deployed against more than 18,500 km, leaving 11,500 km](https://deskeco.com/2026/08/03/transformation-numerique-en-rdc-encore-11500-km-de-fibre-optique-deployer-pour-atteindre-un-total-de) over a 2030 horizon; the minister attributes the pace to budget constraints and a low disbursement rate on external financing. Around it, [eastern DR Congo is tied into the East African terrestrial corridor](https://itweb.africa/article/paratus-lights-up-new-east-africa-fibre-highway-linking-goma-to-mombasa/wbrpOqg2aJQMDLZn) and the internet exchange now operates as a [distributed exchange across two carrier-neutral facilities](https://itweb.africa/article/distributed-exchange-boosts-drc-connectivity/4r1ly7R9GkLvpmda), with no traffic or member figure held at any date.

Two announced programmes have not moved off the announcement. A rural joint venture's [commitment to 2,000 solar base stations was restated with a 2031 date](https://www.connectingafrica.com/connectivity/drc-connectivity-improves-but-some-usage-power-supply-gaps-linger) and carries no sites-built count; a [sovereign satellite estimated at US$400m](https://actualite.cd/2025/08/31/la-rdc-se-lance-dans-un-projet-dacquisition-dun-satellite-cout-estime-400-millions-de) has no contract, operator or launch date, and the claim that a bank has mobilised the funds is relayed by the presidency and uncorroborated.

Commercial data-centre capacity is specified: [400 racks over 1,542 square metres at 1.5 MW on US$30m invested](https://actualite.cd/2025/10/31/infrastructures-la-rdc-franchit-un-cap-decisif-dans-sa-transformation-numerique-grace), with no utilisation figure. State capacity is an appropriation: the 2026 finance law carries [FC 26,756,546,346 for national data-centre acquisition](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf) with no procurement or site on file.

The state operator's position is financial: on current finances it [could theoretically pay salaries only every four months](https://rdc-monde.com/scpt-ce-que-lon-ne-dit-pas-sur-les-efforts-de-redressement-en-cours/), which management says rigour has brought to every two to two and a half months. No incident-response capability, technical-capacity base or energy provision for digital infrastructure is held.
<!-- /narrative -->

## Digital public infrastructure

| System or instrument | Status | As at |
|---|---|---|
| Airport levy electronic payment, Kinshasa province | [Planned](https://rdc-monde.com/aeroport-de-ndjili-le-paiement-electronique-du-go-pass-annonce-des-juillet/) | The provincial finance minister made electronic payment of the levy official from July |
| Biometric passport | [Implemented](https://www.biometricupdate.com/202506/drc-cuts-biometric-passport-price-adds-online-enrollment-with-new-supplier-dermalog) | The price cut to US$75 with a new supplier and online enrolment |
| Business-creation single window | [Implemented, one segment only](https://actualite.cd/2026/07/28/formalisation-des-entreprises-en-rdc-daniel-mukoko-samba-mise-sur-le-numerique-pour) | A ministerial inspection at which the new portal and mobile application were presented |
| Card-and-wallet interoperability service | [Implemented](https://visa-inc.africa-newsroom.com/press/visa-pay-launches-to-expand-access-to-digital-payments-across-the-democratic-republic-of-the-congo-drc) | A card-network interoperability service launched for the country through a payments aggregator |
| Central bank de-dollarisation deadline | [Planned](https://actualite.cd/2026/05/04/inclusion-financiere-en-rdc-58-dacces-contre-25-30-de-bancarisation-la-bcc-parie-sur-la) | The central bank set 9 April 2027 as the date from which no foreign-cash transaction of any amount is permitted |
| Data-sharing platform and government payment gateway | [Planned](https://www.biometricupdate.com/202605/drc-seeks-consultant-for-ambitious-digital-transformation-dpi-project) | Named as deliverables of the digital transformation project in the consultant tender |
| Digital government platform | [Planned, target missed](https://www.biometricupdate.com/202601/drc-envisages-a-functional-digital-government-platform-by-march) | A March 2026 target passed with neither launch nor delay announced |
| Economic-inspection document authentication platform | [Piloting](https://actualite.cd/2026/06/26/rdc-le-ministere-de-leconomie-nationale-mise-sur-le-numerique-pour-renforcer-la) | Technical training of ministry staff closed after a session running 9 to 24 June 2026 |
| Higher-education portal and national digital library | [Implemented](https://www.wearetech.africa/fr/fils/actualites/tech/rdc-centralisation-des-donnees-et-acces-au-savoir-les-enjeux-du-nouveau-portail-numerique-universitaire) | The portal and a first national digital library went live under external financing |
| Kinshasa drone surveillance and citizen-reporting application | [Implemented](https://rdc-monde.com/kinshasa-investit-dans-les-drones-entre-lespoir-et-le-doute/) | A drone and citizen-reporting deployment reported, operated through a private company |
| Microfinance and cooperative digitalisation platform | [In development](https://actualite.cd/2026/04/16/rdc-le-projet-transforme-appuie-la-digitalisation-des-imf-et-coopec) | A partnership agreement signed in Kinshasa between a payments firm and two sector associations |
| National e-government strategy 2025-2030 | [In development](https://www.biometricupdate.com/202605/drc-seeks-consultant-for-ambitious-digital-transformation-dpi-project) | Named in the digital-infrastructure tender, with its master plan and governance framework as the consultant's deliverables |
| National electronic certification authority | [Planned](https://actu7.cd/2026/07/29/augustin-kibassa-maliba-lance-le-chantier-dun-cadre-strategique-pour-accelerer-la-transformation-digitale-de-la-rdc/) | Named by the minister among the instruments to be articulated into the governance framework |
| Palm-vein biometric payment terminal | [Piloting](https://actualite.cd/2026/04/30/rdc-lancement-de-mainmoney-une-technologie-monetaire-pour-eviter-la-population-du-poids) | The palm-vein payment technology launched in Kinshasa |
| Prepaid card partnership | [Planned](https://techafricanews.com/2026/07/14/mastercard-and-flash-partner-to-expand-digital-financial-inclusion-in-drc/) | A card network and a domestic payments company announced physical and virtual prepaid card issuance |
| Public-enterprise digitalisation plan 2026-2028 | [Planned](https://rdc-monde.com/rdc-finis-les-circuits-lents-ligf-passe-en-mode-numerique-pour-traquer-la-fraude-et-booster-les-entreprises-publiques/) | A dissemination seminar for public managers opened in Kinshasa with digitalisation as the plan's backbone |
| RDC-PASS national digital identity platform | [Piloting](https://www.biometricupdate.com/202606/drc-launches-rdc-pass-digital-id-platform-under-20-year-ppp-with-trident) | A state launch ceremony in Kinshasa, with rollout to begin with technical audits and pilot testing |
| Second general population and housing census | [In development](https://actualite.cd/2026/07/11/journee-mondiale-de-la-population-le-rgph-2-au-coeur-de-la-strategie-du-gouvernement) | The planning minister announced recruitment of mapping agents and fixed general enumeration for July 2027 |
| Second national identification and card issuance | [In development](https://actualite.cd/2026/05/29/identification-nationale-de-la-population-en-rdc-lonip-annonce-le-demarrage-des) | The identification office states operations are about to start, financed by a partnership because the 2026 finance law fell short |
| Single social register | [Planned](https://actualite.cd/2025/07/27/rdc-adoption-du-projet-de-decret-portant-creation-organisation-et-fonctionnement-du) | A draft decree creating and organising the register adopted at the 53rd Council of Ministers |
| Stablecoin settlement of mobile-wallet top-ups | [Piloting](https://www.theeastafrican.co.ke/tea/business-tech/visa-tests-stablecoins-in-drc-for-cross-border-payments-5515040) | A card-network executive confirmed on record a live use case settling wallet top-ups in stablecoins in the background |
| Use of satellite or earth-observation data in public administration | ***Not held*** | — |

<!-- narrative: dpi -->
Identity is being built twice, in parallel, on different legal bases. The [RDC-PASS platform launched on 13 June 2026 under a twenty-year exclusive know-your-customer concession, valued at US$97.1m with US$2.6m raised by the concessionaire](https://www.biometricupdate.com/202606/drc-launches-rdc-pass-digital-id-platform-under-20-year-ppp-with-trident) for initial rollout. Separately the identification office states [operations are about to start on a second national identification, financed by a partnership because the 2026 finance law fell short](https://actualite.cd/2026/05/29/identification-nationale-de-la-population-en-rdc-lonip-annonce-le-demarrage-des) — the appropriation for national identification acquisition is [FC 20,654,178](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf), a figure that cross-foots exactly against its group total. The last national identification was in 1984. A [bill setting one legal frame for identification data was tabled on 12 June 2026](https://actualite.cd/2026/06/12/assemblee-nationale-venance-eyanga-et-patrick-namazihana-deposent-une-proposition-de-loi) and nothing on file connects it to the concession.

Payments are the most active layer and the least measured. A [card-and-wallet interoperability service](https://visa-inc.africa-newsroom.com/press/visa-pay-launches-to-expand-access-to-digital-payments-across-the-democratic-republic-of-the-congo-drc) is live and now carries a [stablecoin settlement of wallet top-ups](https://www.theeastafrican.co.ke/tea/business-tech/visa-tests-stablecoins-in-drc-for-cross-border-payments-5515040), which runs against the central bank's own deadline of [9 April 2027, from which no foreign-cash transaction of any amount is permitted](https://actualite.cd/2026/05/04/inclusion-financiere-en-rdc-58-dacces-contre-25-30-de-bancarisation-la-bcc-parie-sur-la). Account numbers cannot be stated: the base carries [24m, 29m, 30m and 35m across sources](https://www.connectingafrica.com/financial-inclusion/how-onafriq-is-growing-digital-finance-amid-uncertainty-in-drc), none reconcilable and no central-bank series published.

Government platforms are specified on paper. The [digital government platform missed a first-quarter 2026 target with neither launch nor delay announced](https://www.biometricupdate.com/202605/drc-seeks-consultant-for-ambitious-digital-transformation-dpi-project); the e-government strategy dated 2025 to 2030 has its master plan and interoperability framework still to be defined by a consultant not yet appointed. Where delivery exists it is narrow: [sole-trader attestations issue in under 48 hours while full online company creation is not possible](https://actualite.cd/2026/07/28/formalisation-des-entreprises-en-rdc-daniel-mukoko-samba-mise-sur-le-numerique-pour).
<!-- /narrative -->

## Governance and regulation

| System or instrument | Status | As at |
|---|---|---|
| Bill on access to information and transparency in public life | [In development](https://actualite.cd/2026/03/28/assemblee-nationale-la-proposition-de-loi-sur-lacces-linformation-et-la-transparence-de) | Declared admissible by the National Assembly 2026-03-27 and referred to a joint commission with ten days to report |
| Bill on the identification of natural persons | [In development](https://actualite.cd/2026/06/12/assemblee-nationale-venance-eyanga-et-patrick-namazihana-deposent-une-proposition-de-loi) | Tabled at the National Assembly bureau 2026-06-12 |
| Code du numérique | [Implemented](https://actualite.cd/2026/04/21/rdc-un-nouvel-ouvrage-sur-la-protection-des-donnees-personnelles-porte-sur-les-fonts) | Promulgated 2023-03-13 |
| Interministerial order on digital-sector duties, taxes and fees | [Implemented, suspended](https://acp.cd/business/rdc-le-secteur-du-numerique-encore-confronte-a-des-nombreuses-contraintes-un-specialiste/) | In force on signature 2026-07-20 under its article 5; suspended by government communiqué of 2026-08-03 |
| National classification regime for official documents | [Planned](https://actualite.cd/2026/03/20/rdc-vers-linstitutionnalisation-dun-regime-de-classification-et-de-protection-des) | An information note presented to the 82nd Council of Ministers 2026-03-13 |
| National data protection authority | [Planned](https://www.wearetech.africa/fr/fils/actualites/tech/numerique-la-rdc-met-en-place-un-reseau-de-responsables-de-la-protection-des-donnees) | No decree defining its organisation and functioning found |
| National digital plan 2026-2030 | [In development](https://www.wearetech.africa/actualites/fils/actualites/gestion-publique/numerique-la-rdc-annonce-l-operationnalisation-d-un-financement-de-500-millions) | The government announced operationalisation of a US$500m partner tranche after talks with two development financiers |
| Regional free-roaming resolution | [Planned](https://techreviewafrica.com/news/4133) | A resolution adopted mandating effective implementation across Central Africa by 2026-06-30 |
| Reorganisation of the digital mandate in the 2026 finance law | [Implemented](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf) | Enacted 2025-12-29 |
| Satellite internet service-provider licence | [Implemented](https://actualite.cd/2025/05/02/rdc-starlink-obtient-une-licence-officielle-pour-operer-comme-fournisseur-dacces) | Granted by regulator communiqué, reversing a March 2024 prohibition |
| Sectoral digital economy policy 2026-2030 | [In development, conditionally validated](https://www.mediacongo.net/article-actualite-165910_economie_numerique_les_experts_valident_le_projet_de_politique_sectorielle_2026_2030.html) | Validated by experts on 21 and 22 July 2026 subject to completing the costing, indicators and institutional responsibilities |
| Social register decree | [In development](https://actualite.cd/2025/07/27/rdc-adoption-du-projet-de-decret-portant-creation-organisation-et-fonctionnement-du) | A draft decree adopted at the 53rd Council of Ministers 2025-07-25 |
| A named public-debate object with a movable position | ***Not held*** | — |
| EU digital governance or regulatory cooperation | ***Not held*** | — |
| National data governance strategy | ***Not held*** | — |
| National open-data policy or portal | ***Not held*** | — |

<!-- narrative: governance -->
The [Code du numérique has been in force since March 2023](https://actualite.cd/2026/04/21/rdc-un-nouvel-ouvrage-sur-la-protection-des-donnees-personnelles-porte-sur-les-fonts); the data protection authority it requires is [still not created](https://www.wearetech.africa/fr/fils/actualites/tech/numerique-la-rdc-met-en-place-un-reseau-de-responsables-de-la-protection-des-donnees), the minister saying in March 2026 that structuring a professional network precedes its creation. That network is live, and it is not a supervisory authority with sanctioning powers.

Two bills sit in opposite directions. A [bill on access to information was declared admissible on 27 March 2026 and referred to a joint commission with ten days to report](https://actualite.cd/2026/03/28/assemblee-nationale-la-proposition-de-loi-sur-lacces-linformation-et-la-transparence-de), with no commission report four months past that deadline. In the other direction, an information note to the Council of Ministers proposes a [classification regime of four levels with access limited to cleared agents](https://actualite.cd/2026/03/20/rdc-vers-linstitutionnalisation-dun-regime-de-classification-et-de-protection-des).

Fiscal instruments arrive faster than institutional ones. An [interministerial order pricing digital-sector duties took effect on signature on 20 July 2026 and was suspended by government communiqué on 3 August](https://actu7.cd/2026/07/31/rdc-le-gouvernement-fixe-les-nouveaux-droits-taxes-et-redevances-applicables-au-secteur-du-numerique/) — twelve days in force. Neither the gazetted order nor the suspending communiqué is held.

Network loss in conflict-held eastern territories is recorded as [war damage and fuel-access failure rather than any ordered restriction](https://actualite.cd/2026/02/18/les-territoires-de-rutshuru-et-masisi-de-nouveau-connectes-aux-reseaux-airtel-et-orange), with mobile money the first casualty in each outage. No data governance strategy, open-data policy or European Union instrument is held.
<!-- /narrative -->

## Artificial intelligence and the technology sector

| System or instrument | Status | As at |
|---|---|---|
| AI-assisted business-plan platform | [Implemented](https://actualite.cd/2026/02/04/entrepreneuriat-en-rdc-bokeli-la-plateforme-digitale-intelligente-du-fogec-pour-la) | Launched in Kinshasa by a state guarantee fund, built by a domestic firm |
| Congo River fibre-optic memorandum | [Planned](https://www.connectingafrica.com/fiber-networking/drc-plans-1-5b-fiber-optic-project-along-congo-river) | A memorandum signed with a Chinese supplier for a 2,100 km route |
| Congolese academy of artificial intelligence | [Planned](https://www.agenceecofin.com/actualites-numerique/1110-132270-la-rdc-prevoit-1-5-milliard-de-budget-pour-son-plan-national-du-numerique-et-de-l-ia-2026-2030) | Named as a provision of the AI strategy at the national digital plan's drafting launch |
| Digital infrastructure and cybersecurity partnership | [Planned](https://techafricanews.com/2026/04/17/cybastion-partners-with-drc-to-expand-digital-infrastructure-and-cybersecurity/) | A strategic partnership signed on the margins of the spring meetings in Washington |
| Local-content subcontracting enforcement in telecoms | [In development](https://actualite.cd/2026/07/23/rdc-larsp-et-larptc-renforcent-leur-cooperation-pour-promouvoir-la-sous-traitance-dans) | A first working session agreeing a harmonised reading of the texts and joint inspection teams |
| National artificial intelligence strategy | [In development, technically validated and not adopted](https://zoom-eco.net/autres-actualites/rdc-la-strategie-nationale-de-lintelligence-artificielle-franchit-une-etape-decisive-apres-sa-validation-technique/) | A stakeholder workshop of 23 to 27 July 2026 in Kinshasa validated the draft subject to amendments |
| Payments-modernisation claim in a United States federal court | [In development](https://itweb.africa/article/us-fintech-sues-dr-congo-for-400m-over-failed-banking-deal/) | A fintech filed against the state and senior officials over the collapsed banking and payments modernisation project |
| Police cooperation accord | [Implemented](https://actualite.cd/2026/05/27/la-rdc-signe-un-accord-de-cooperation-policiere-avec-la-chine-pour-renforcer-les) | Signed in Beijing by the interior vice-premier and China's public-security minister |
| Startup Act implementing decrees | [In development](https://acp.cd/business/rdc-le-secteur-du-numerique-encore-confronte-a-des-nombreuses-contraintes-un-specialiste/) | A government communiqué of 2026-08-03 confirms the implementing texts remain in preparation |
| Student artificial-intelligence facility | [Planned](https://techafricanews.com/2026/02/20/drc-launches-500-million-digital-transformation-with-world-bank-and-afd-support/) | Announced alongside the operationalisation of the US$500m partner tranche |
| India-DR Congo digital or technology engagement | ***Not held*** | — |

<!-- narrative: ai-tech -->
The national AI strategy was [technically validated at a workshop of 23 to 27 July 2026, subject to amendments, and is not adopted](https://zoom-eco.net/autres-actualites/rdc-la-strategie-nationale-de-lintelligence-artificielle-franchit-une-etape-decisive-apres-sa-validation-technique/). Its four pillars run from governance, ethics and regulation to industrialisation and value creation. The text itself is not held. Officials have said on the record that bodies already acting across digital, research, data, cybersecurity and regulation have [roles not yet sufficiently articulated in a common steering architecture](https://acp.cd/economie/economie-numerique-lusage-responsable-de-li-a-recommande-dans-un-atelier-a-kinshasa/).

Deployment runs ahead of the strategy in one place and behind it everywhere else. [AI was used to mark the 2024-25 state examination](https://actualite.cd/2025/11/27/adis-2025-raissa-malu-presente-levolution-de-la-transformation-numerique-du-systeme), with no scope, vendor, model, accuracy or human-oversight detail held; a state guarantee fund runs a [live AI-assisted business-plan platform](https://actualite.cd/2026/02/04/entrepreneuriat-en-rdc-bokeli-la-plateforme-digitale-intelligente-du-fogec-pour-la) with no usage figures or data-protection arrangement stated.

The startup regime is the clearest stalled position in the base. [Four draft decrees adopted in May 2025 to operationalise the fiscal, customs and exchange advantages of the 2022 ordinance-law remain in preparation](https://acp.cd/business/rdc-le-secteur-du-numerique-encore-confronte-a-des-nombreuses-contraintes-un-specialiste/), so no exemption regime for startups or digital small businesses is in force.

External technology relationships are memoranda rather than works. A Chinese supplier memorandum covers [about 2,100 km along the Congo River at a stated US$1.5bn, with more than US$400m required for the first phase](https://www.connectingafrica.com/fiber-networking/drc-plans-1-5b-fiber-optic-project-along-congo-river) and no financing close; a [United States partnership covers data centres, connectivity, cybersecurity, financial technology and artificial intelligence](https://techafricanews.com/2026/04/17/cybastion-partners-with-drc-to-expand-digital-infrastructure-and-cybersecurity/) with no value or milestones. A [claim in a United States federal court seeks up to US$400m over a collapsed payments-modernisation project](https://itweb.africa/article/us-fintech-sues-dr-congo-for-400m-over-failed-banking-deal/); the government calls the signed documents a non-binding memorandum, and no ruling is held.
<!-- /narrative -->

## Inclusion, capacity and finance

| System or instrument | Status | As at |
|---|---|---|
| Central bank card-network memorandum | [Implemented](https://techafricanews.com/2026/04/16/new-visa-and-bcc-agreement-targets-financial-inclusion-and-payment-modernization-in-congo/) | A memorandum signed in Kinshasa by the central bank governor and a card-network executive |
| Digital economy section appropriation | [Implemented](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf) | The 2026 finance law enacted, renaming the section and fixing its domestic credits |
| Employment and skills platform | [Implemented](https://rdc-monde.com/onem-lancement-dune-application-numerique-mikuba-pour-mieux-cartographier-les-competences-et-les-besoins-des-entreprises/) | The employment office launched a portal and mobile application built with external technical and financial support |
| National digital plan human-capital targets | [Planned](https://www.biometricupdate.com/202601/drc-envisages-a-functional-digital-government-platform-by-march) | Plan details published setting numeric training targets under the human-capital pillar |
| National identification system appropriation | [Implemented](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf) | Appropriated inside the digital section's domestic investment group |
| National policy on information technology in education and training | [Implemented](https://actualite.cd/2026/06/15/rdc-le-gouvernement-se-dote-dune-politique-nationale-des-technologies-de-linformation-et) | Adopted by the 92nd Council of Ministers on the education minister's presentation |
| Posts and telecommunications section appropriation | [Implemented](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf) | The 2026 finance law enacted, stripping new technologies from the section title and fixing its domestic credits |
| Rural civil-servant payroll | [In development](https://actualite.cd/2026/07/24/paie-des-agents-publics-la-cadeco-et-le-csp-prets-lancer-la-paie-de-proximite-en-milieu) | A state bank and the payroll monitoring committee settled technical and organisational terms at a Kinshasa working session |
| Tower operator investment convention | [Implemented](https://actualite.cd/2026/03/31/rdc-signature-dun-investissement-de-plus-de-100-millions-usd-entre-lanapi-et-helios) | An investment convention of more than US$100m signed with the investment-promotion agency |
| Tower platform senior facility | [Implemented](https://www.eaif.com/eaaif-accelerates-africas-digital-transformation-with-a-combined-usd-82-8-million-commitment-to-eastcastle-drc-and-liquid-intelligent-technologies/) | A development fund committed US$32.8m into an upsized US$179m senior secured facility |
| Universal service fund pilot sites | [Piloting](https://actualite.cd/2026/06/02/kongo-central-le-fdsu-lance-ses-premiers-sites-pilotes-de-connectivite-et-pose-les-bases) | The first pilot connectivity sites launched in Kongo Central, with the foundation stone laid by the posts and telecommunications minister |
| Domestic public funding for digital and AI research | ***Not held*** | — |

<!-- narrative: inclusion -->
The digital economy section's domestic appropriation is [FC 81,698,223,741 for FY2026, up 34.8% on FY2025 but 6.2% below FY2024](https://www.budget.gouv.cd/wp-content/uploads/budget2026/lf_2026_depenses_final.pdf); on the export's own conversion the line has fallen in dollars. No FY2026 execution has been published and no audited outturn is obtainable at section grain.

The universal service fund moved from an instrument civil society described as unavailable to a construction site: the [first pilot connectivity sites were launched in Kongo Central](https://actualite.cd/2026/06/02/kongo-central-le-fdsu-lance-ses-premiers-sites-pilotes-de-connectivite-et-pose-les-bases) with a community digital-learning centre attached. No site count, budget or appropriation line for the fund is held.

Financial inclusion is stated once and without a trajectory: [about 58% against 25 to 30% bank-account penetration](https://actualite.cd/2026/05/04/inclusion-financiere-en-rdc-58-dacces-contre-25-30-de-bancarisation-la-bcc-parie-sur-la), the gap attributed to mobile-money use without a bank account. No earlier dated rate is held.

Training targets exist without enrolments. The national digital plan sets [250,000 young people to be trained in artificial intelligence, cybersecurity and programming, at least 3,000 in advanced digital skills and about 1,000 teachers, all to 2030](https://www.biometricupdate.com/202601/drc-envisages-a-functional-digital-government-platform-by-march); no source establishes a single enrolment against any of them, and no domestic public funding for digital or AI research exists in the base. The largest single training cohort held is [41 certificates awarded in November 2025](https://actualite.cd/2025/11/05/formation-numerique-en-rdc-femintech-octroie-41-brevets-aux-nouveaux-formes-en).
<!-- /narrative -->

## Gaps to fill

| System or instrument | What would settle it | Last probed |
|---|---|---|
| Functioning national incident-response capability | a founding instrument for a computer emergency response team, an incident report or advisory, or an international cybersecurity assessment for the country | not yet probed |
| National technical-capacity base | a named compute or high-performance computing facility, a national technical or testing institution with a dated status, or a skills-for-infrastructure programme with enrolment figures | not yet probed |
| Energy provision for digital infrastructure | a utility-operator power agreement, a dated telecom-site electrification programme with site counts, or a data-centre power-purchase arrangement | not yet probed |
| National data governance strategy | the strategy document or the decree adopting it | not yet probed |
| National open-data policy or portal | a decree creating an open-data portal, a statistics-institute microdata release policy, or the access-to-information bill once adopted | not yet probed |
| A named public-debate object with a movable position | a recurring dated government position statement, a published ministerial roadmap with review dates, or a national consultation with a stated timetable | not yet probed |
| EU digital governance or regulatory cooperation | the EU financing agreement or gateway package document, or an EU delegation statement on digital regulation, data adequacy or governance support | not yet probed |
| Use of satellite or earth-observation data in public administration | a statistics-institute, planning-ministry, land-administration or agriculture-ministry statement naming an earth-observation data source or a satellite-imagery procurement | not yet probed |
| India-DR Congo digital or technology engagement | an India-DR Congo memorandum, line of credit, training placement or Indian supplier contract naming the country | not yet probed |
| FY2026 budget execution | the budget ministry's monthly or quarterly execution statements for the 2026 exercise | not yet probed |
| Audited outturn for the digital sections | the audit court's general report or the settlement law for FY2024, at section grain | not yet probed |
| Gender-disaggregated connectivity or usage | a mobile-industry gender-gap country cut, or a statistics-institute or central-bank survey with sex-disaggregated tables | not yet probed |
| National digital literacy rate or skills baseline | an international digital-skills indicator for the country, a household survey module, or the sectoral policy's baseline annexe | not yet probed |
| Domestic public funding for digital and AI research | a finance-law section or programme line for scientific research, or the AI strategy's costed roadmap | not yet probed |

<!-- narrative: gaps -->
Ten instrument rows carry no reliable statement, and they cluster where the state would have to publish something about itself. Four are institutional or technical: no incident-response capability, no technical-capacity base, no energy provision for digital infrastructure, and no data governance strategy — the last named by the minister on 29 July 2026 as an instrument the state already holds, with no text or adoption date on file. Three are about openness: no open-data policy, no European Union instrument, and no public-debate object with a movable position.

Two sit in the finance layer and bear on every figure above: no FY2026 budget execution and no audited outturn at section grain. Two measures — a digital literacy rate and any gender-disaggregated connectivity figure — are absent from the base. Each row is in `gaps.csv` with what would settle it; none has yet been probed.
<!-- /narrative -->
