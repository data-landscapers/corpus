...
*(Spec for the domestic-budget work: what is being measured, which point of the budget cycle a figure is, and what a record has to carry. It keeps the rules from OSINT's `documentation/domestic-budget-extraction.md`; the incidents stay there, published in the public process mirror, [data-landscapers/osint-process](https://github.com/data-landscapers/osint-process). Corpus's `BUDGET-EXTRACT` runbook is written against this file. Where it and OSINT's driver `wiki/finance-load-domestic-state.md` disagree on a field, the driver wins.)*
...
**Scope by the line's text, never by its label.** A *programme de digitalisation* can hold a national radio station. The inverse is commoner: the largest identity appropriation is usually called *Civil Registration*, *<Ministry> Computerisation* or *Passport Administration*, which no keyword reaches.

**Generic office hardware is overhead, not a digital-activity line.** Computers, network maintenance and IT equipment purchases are excluded; software, systems, digitalisation, connectivity and biometric lines are recorded. **A ministry's own IT unit** — an information-systems directorate, IT division or ICT cost centre printed as a line of its own — records at `partial` *(2026-09-25)*: it mixes the ministry's systems, which are in the frame, with desktop support and equipment, which are not. A line within it that names a system or software records at `whole`; its equipment, supplies and fuel lines do not record.

**The sector vote is never the whole, and a cross-vote scan is not optional**: digital money outside the digital ministry has run from about 60% of the sector vote to more than double it, often in interior, finance, justice or the head of government's vote. The scan **locates and does not identify**: in a stacked layout a keyword and an amount five lines apart belong to different rows, so read the whole vote block and pair by position before recording anything from it.
...
**A headline is untrustworthy without the split**: external money moving can hide the domestic programme moving the other way.
...
**MTEF outer years and plan-period totals are not records.**
...
**Cross-foot before recording. A table that does not cross-foot is not a table that may be published.** Arithmetic catches what inspection misses.

**Scale is a property of the table, not of the document.** One volume can print units, thousands, millions and percent of GDP in different tables, and narrative and annex can disagree.

**Layout drift is the commonest silent failure.** Codes, labels and amounts are independent vertical stacks paired only by position, so a text dump plausibly attaches a row to its neighbour's money. **Bind by page geometry**, clustering words or characters on their vertical position. Where a library returns a number one digit shorter than the page prints, it is dropping glyphs — read at character level.

**`pdftotext -table` beats `-layout`** on recapitulative and summary tables. **Always extract with `-enc UTF-8`**: the default is Latin-1, and every accented search term then returns zero, which looks like a finding. Corroborate any "zero occurrences" claim with an unaccented term.
...
**Machinery-of-government changes break the code series, and they are the norm**: ministries split and merge, programmes are renumbered, codes reissued, perimeters moved. Any series keyed on a code needs an explicit mapping row at the date of the break, and a body-level jump says nothing about the digital line inside it.
...
An envelope: a ministry total, a thematic total that spans ministries, or a body whose mandate is mixed. An MTEF, plan-period or indicative figure. A procurement plan or a signed contract — neither is an appropriation, and a plan re-based on the enacted vote is the same money under two naming systems. A tax expenditure: foregone revenue is not spend, though no account of the budget should omit it. State revenue — privatisation proceeds, spectrum sales, the receipts a regulator mobilises for the treasury. A commitment plan. A line whose funder is named only as external. And never a parent and its own children together.

## An absence is a finding, and it has to be earned

**A nil return that searched only the sector vote is not a nil return.**

**A body invisible in the estimates may still be funded** — inside another ministry's vote, by statutory levy outside the vote, by an own-source regulator, or named only in a supply speech, a parliamentary answer or the framing letter's chart of accounts. Establish which before writing "unfunded".

**A complete enumeration is what makes an absence evidence.** Where the library was walked end to end and the term is not in it, the absence is a dated finding saying what was searched. Otherwise the result is only that nothing was found.

**The absence is often the story** — a data-protection authority enacted and never funded, a registry built entirely with other people's money — and is reached by reading what the budget does not say.
