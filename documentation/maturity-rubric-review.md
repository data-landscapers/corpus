...
**Non-state mobilisation counts an equity round of unknown status** *(CC, 2026-09-24, from the SYC draft)*. The compile left Seychelles' Fusepay pre-seed round with `status: Unknown`, so the row read zero while the base held the round. An equity round is reported when it closes, so it is money that moved. A loan or a grant of unknown status may still be a pipeline announcement, and stays out. Two countries change, SYC and COG; eight rows across the estate carry the status.

**A reference figure that jumps is suspect** *(CC, 2026-09-24, from the TCD draft)*. WDI gives Chad's rural electricity access as 0.4 % in 2023 and 100 % in 2024, against 13.4 % nationally. A percentage figure that moves more than 30 points from the country's figure no more than two years before it is marked `suspect` in `reference/measures.csv` and never used (`maturity-reference.py` `suspect()`). Six figures are caught: TCD 2024, BFA's 2024 STEM share, GHA's 2020–21 internet use (ITU's series shifted that year), and ZAF's 2016–17 mobile ownership. Each country falls back to its last good figure, or a later one past the shift.
