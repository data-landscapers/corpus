...
The JS then reproduces `csv.DictWriter`'s output: `QUOTE_MINIMAL` quoting, `; ` between list values, Python's `True`/`False` for the `finance` boolean, **CRLF line endings and a leading BOM**. That last pair is the one worth stating, because `RENDER.md` → *The finance tables* is the standing record of what line endings do under a Cowork build when two sides disagree about them.

**The BOM arrived on 2026-08-25 and it is the file's property, not the export's** *(Bill, who asked whether the CSVs needed one or whether something was wrong upstream)*. Nothing was wrong upstream: `lint-mojibake.py` reports clean over inputs and derived outputs alike, and the 26,000 non-ASCII characters in this file are correctly encoded French, Portuguese and Arabic. Excel on Windows was guessing, because a BOM-less UTF-8 CSV opens in the ANSI codepage — so `República` arrived as `RepÃºblica` on the largest download on the site while every finance CSV beside it opened clean, those having been written `utf-8-sig` from the start. `build-catalogue.py` now writes one; `toCSV` prepends one so a reader's filtered cut does not open worse than the whole file it came from; and the byte comparison below is what stops the two drifting apart again.
...
**The byte test was green while the feature was broken, and that is worth stating** *(2026-08-24)*. A reader whose browser held a payload cached from before `cols` existed got a CSV of thirty blank lines — no header, one empty row per record — because `toCSV` walks `CSVCOLS` and an empty spec produces an empty row rather than an error. The JSON export beside it was perfect, because the JSON path never touches `CSVCOLS`. The test could not see it: it reads `cols` out of the payload it was just built with, so it was checking the serialiser against a spec that was, by construction, present. **The test proves the serialiser; it cannot prove the spec arrived.** `catalogue.py`'s `stamp()` is the fix — the payload's URL now carries its own content hash, so a page and its data can no longer come from different builds — and `exportSelection` refuses with a message rather than writing a blank file if it ever happens again. A silently empty export is the worst failure this page has, because it is the only one that leaves the building looking like a real file.
