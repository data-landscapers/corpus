...
**The sending domain is a subdomain, and the alerts build had chosen the root** (`archived/catalogue-alerts-build.md` A3)**.** Buttondown's manual flow put the records on `newsletter.` instead, and it is the better answer for the reason A3 had already flagged as live: newsletter reputation now sits on its own name rather than on the domain carrying Bill's M365 correspondence. The cost is that the `From` address reads `newsletter.data-landscapers.io`.
