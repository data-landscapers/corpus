...
**What is deliberately not done.** The 241 published report and topic documents keep the old chrome and its three dead links until each is re-cut, per `RENDER.md` (`--force` = 241 editions). `content/method.md` was deleted and `content/methodology.md` added by an earlier session, so `method.py` cannot build; that rename is not this work's to finish.
