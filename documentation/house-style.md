...
`scripts/methodology.py` builds `/methodology/` and its three annexes — `/methodology/document-lifecycle/`, `/methodology/process-inventory/` and `/methodology/lookups/` — from four files in `content/`, with anchor ids on every `##`, an `<h1>` from the page's title rather than from the markdown, and one or two `.article-toc` bars above the body: the other pages of the set, then this page's own sections. `/method/` is a hand-written redirect stub kept because the old URL sits in the baked chrome of pages published before the rename.
