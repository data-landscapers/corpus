...
`scripts/methodology.py` builds `/methodology/` and its three annexes — `/methodology/document-lifecycle/`, `/methodology/process-inventory/` and `/methodology/lookups/` — from four files in `content/`, with anchor ids on every `##`, an `<h1>` from the page's title rather than from the markdown, and one or two `.article-toc` bars above the body: the other pages of the set, then this page's own sections. Their page-type sheet is `methodology.css`, whose whole content is the process inventory's three column widths — the filenames in column 2 are unbreakable words that auto layout hands 40% of the table to, so the widths are fixed and the builder puts a `<wbr>` after every `/` and `-` inside a `<code>` span. `/method/` is a hand-written redirect stub kept because the old URL sits in the baked chrome of pages published before the rename.

**After any chrome or stylesheet change, the tool is `render.py --repage`.** It rewrites the served page under the edition already held, touches no PDF and mints nothing. `--force` is the wrong instrument for a presentation change: it would cut a full set of new dated editions to fix a navigation bar. The distinction: the dated PDF is the citable artefact, while `AGO-status.html` is an undated view nothing can cite the bytes of.
...
`chrome_lib.external_links()` applies it as a **post-pass over the finished page**, not as a rule in each emitter. Anchors are written from nine places — the six page builders, `render.py`'s two passes, and the Markdown converter that turns a report's source links into HTML — and the last of those is inside a library with no call site to reach. So the rule runs where the page is one string and every anchor in it is visible at once. `target="_blank"` carries `rel="noopener"`; it does not carry `noreferrer`, because a publisher we send a reader to should see where that reader came from. An anchor that already states a `target` is left alone — the catalogue's result rows and `datatable.js`'s source column set their own. `scripts/test_external_links.py` holds the cases, in both directions: an internal link that opens a tab is as wrong as an outbound one that does not, and neither is visible in a diff of 55,000 anchors. **`scripts/lint-external-links.py` is what notices when a builder stops calling it** — it asserts the rule on the built tree and, a render earlier, on any script that takes its chrome from `chrome_lib` and writes a page without the post-pass. RENDER runs it at Step 7, before the push. **The main site applies the same rule in the browser** *(2026-09-11)* — GitHub Pages runs no custom plugins, so a script at the foot of `_layouts/default.html` sets the target on load, with the same three hosts. Change one list and you change both.
