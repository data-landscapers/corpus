...
**An indicator hangs off a taxonomy Level-2 subject, and the subject must exist.** `indicators_lib.frame()` joins `Topic L2` to `lookups/taxonomy.csv` for the chapter and the display order; an unknown subject breaks the render. The vocabulary is **OSINT's**: its `lookups/taxonomy.md` is what sources are tagged against and what its lint checks. `lookups/taxonomy.csv` is Corpus's own display copy — order, Level-1 group, label — and gains the row only once the mirror shows the subject.

So a new indicator either sits under an existing subject — preferred, because it is a Corpus-only change — or needs a new subject, which is an OSINT change: `scripts/osint-patch.py prepare`, add the slug to `lookups/taxonomy.md` (and to `lookups/report-region-sections.csv`) in the clone, `cut`, deliver to `prepared/` on the share, and write the `[ACT]` note that asks OSINT to apply it — the concept page the new slug needs is wiki prose, which a patch may not carry, so the note names it as OSINT's to mint. **Nothing in the frame is minted until the mirror shows the subject.** Sources will need the new tag before the coverage byproduct can find anything for it; that is a sweep brief on Bill's call, never a Corpus-side write.
