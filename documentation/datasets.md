...
| **T2** | **Import.** Move the master from `prep/` into `outputs/datasets/data-centres/`, split the markdown links in `source_urls` into bare URLs separated by `; ` (the comments column keeps its links), and write `scripts/datasets_lib.py` for read, write, the log and ID minting. IDs are never reissued: a closed facility is `retire`d, and its row stays with status `Decommissioned`. **The values are normalised to metadata.csv at the same time**, and each fix goes in the log: `Actiive (under construction)` → `Under construction`; the two spellings of *Designated national / government data centre* → one; blank hyperscaler flags → `No`; `TRUE`/`FALSE` → `Yes`/`No`; a blank `chinese_involvement` → `None identified`; `None found` → `None identified`; `ultimate_parent_hq_country` from names to ISO3, `|`-joined where control is joint (`UK / Zimbabwe` → `GBR|ZWE`). **Two data defects go on T6's list, not T2's**, because each needs a source read to fix: 24 rows have `hyperscaler_relationships` saying *None* but carry a `Yes` flag (the v2 negation filter missed them), and 63 Operational rows have no `year_operational`. | master, lib, empty `logs/dataset-updates.csv` |
