...
- **Coverage gap: North Africa had no rows in v2. Restored 2026-09-21 (Bill).** The first version of the dataset on the main site (`data-landscapers.io/assets/data/data-centres.csv`) held 378 rows. v2 kept 306, with the same IDs and names, and dropped the 71 North African rows and ETH-009. `scripts/datasets-import-dc-v1.py` brought the 71 back through T2's normalisation. ETH-009 stays out, taken as v2's decision. v1 had no `control_*` columns, so control is derived from the ultimate parent's country (84% agreement with v2's judgements), with `control_confidence` blank until T6 assesses it. **Still no rows: Eritrea, São Tomé and Guinea-Bissau** (T8).
...
| **T8** | **Coverage: the three countries with no rows** (ERI, STP, GNB), plus a completeness check on the 71 restored North African rows, which are older than v2 and have not been reviewed since. T7 covers what raw/ already holds. On top of that, one Exa brief per country for operational, planned and under-construction facilities and their operators. The sources go through T5's route. | new rows |
