...
**Then the layering, on Bill's word.** The report layer now resolves its citations through `outputs/catalogue/raw-catalogue.csv` rather than the index — Corpus's own committed table, the same file a reader can download, built by stage 2 from the index rather than being the index. Checked before switching, not after: the two agree exactly — 9,404 URLs each, identical maps, over all 5,189 slugs the 57 ledgers cite, with nothing cited that only the index held. Re-rendering KEN afterwards reports all three documents `unchanged`, which is the strongest form the check can take.

What that trades is a cache that checked its own freshness for one that must be checked, so the check is explicit: `build-catalogue.py` stamps what it was built from (`outputs/catalogue/catalogue-stamp.json` — records and newest `raw/` mtime, read *before* the index refresh so the conservative direction is the one that errs), and `report-render.py` recomputes it in 0.23s and raises `StaleCatalogue` rather than resolve against a table `raw/` has moved past. Counted as well as timed, because a deleted record moves no mtime and a catalogue still listing it resolves a slug the base no longer holds — note 4's dangling citation, reached from inside Corpus this time. Both branches of the guard were tested by doctoring the stamp.

The index is not gone, and should not be: the shape check (§7) counts sources per month, and for a region traverses `wiki/` for scope, neither of which the catalogue carries. So the index is now what its own docstring always called it — local scaffolding for stage 2 and the shape line — and the published artefact is what the layer cites. Stage 2 becomes a precondition of stages 4 and 5, recorded in `BUILD.md`.

One deliberate narrowing came with it: the catalogue lists records, so the 217 artefacts under `raw/` are no longer slugs check M will accept. That is §8's own reading — an artefact is not a citable record, which is exactly what note 7 says about the Comoros budget PDFs — and no ledger cites one.
