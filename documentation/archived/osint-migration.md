...
*(**R8 must not be actioned from this file, 2026-08-22.** It landed in OSINT on 2026-08-16 and is finished; what changed is that the FreeFileSync pass now at the end of `SWEEP-CYCLE` supplies CORPUS's read-only copy of the vault rather than a backup, so a session reading R8's *"a session should not restore any of them"* cold would cut off its own evidence. See the annotation at R8 and `notes-for-corpus.md` note 5.)*
...
CORPUS reads OSINT **read-only** — `raw/`, `lookups/` and the internal `wiki/`, and *not* `index/`, which it builds for itself — and writes only into its own tree. It never writes to OSINT, in any form. **OSINT does not read CORPUS at all**, and gains no publication step in its night. *(Corrected 2026-08-16: this named one exception, the request feed of task R9. There is none — an OSINT session cannot see `C:\CORPUS`, and does not need to. R9 is withdrawn.)*
...
**R8 — DO NOT ACTION. The mirror it retires is now CORPUS's supply of evidence, not a backup** *(2026-08-22, `notes-for-corpus.md` note 5; the task itself landed in OSINT on 2026-08-16 and is finished — this annotation is about re-reading it, not re-running it)*.

Two sentences below are the trap, and both were true when written. *"All backups are CORPUS's… OSINT runs none and keeps none of its own… a session should not restore any of them"* was a statement about **backups**, and on 2026-08-20 the FreeFileSync pass at the end of `SWEEP-CYCLE` stopped being one: it now syncs `C:\OSINT` to `O:\` (`\\bill-vivobook\osint`, which resolves back to this machine's `C:\OSINT`) to supply **CORPUS's read-only copy of the vault**. A CORPUS session actioning R8 from this file would remove the step that feeds its own build — the one direction of error nothing downstream could report, because the build would keep succeeding on stale evidence. OSINT's copy of this note is annotated to the same effect.

**What R8 correctly retired stays retired**: OSINT's own `mirror.bat`, its `Repo-mirrors.ffs_batch`, the old close step and `LINT` #19. The FFS pass now in `SWEEP-CYCLE` → *Mirror* is a different pass with a different job, added four days later, and is not on this list.

**The replacement #19 asks for below is built** *(2026-08-22)*: `scripts/lint-osint-freshness.py`, run at `BUILD.md` stage 0, reads three clocks off the mirror and reports how old CORPUS's evidence is. That closes the gap the next paragraph names — but on the **reading** side, which is CORPUS's, rather than by repointing an OSINT lint at a CORPUS file.

**The task as run on 2026-08-16, kept verbatim below.** CORPUS's `mirror.bat` now backs up **both** repos: OSINT and CORPUS working trees + `git bundle` to Dropbox, plus one FreeFileSync pass (`repos-to-flash.ffs_batch`) copying both repos to `D:`. Remove the `run mirror.bat` close step from `SWEEP-CYCLE`, and the `## Mirror` section that documents it, and retire OSINT's `mirror.bat` + `Repo-mirrors.ffs_batch`.

**Half of this has already happened, and the half that has not is live** *(checked 2026-08-16 — do this one ahead of the rest of the list)*. Both files are gone from OSINT's working tree but still tracked: `git status` shows them as unstaged deletions. `SWEEP-CYCLE` meanwhile still carries `run mirror.bat` as the last command of the night, so the cycle's close step points at a file that is not there. Commit the deletion and cut the step and the section together. **`LINT` #19 is already false-alarming on this**: it compares the newest line of `logs/mirror_log.md` against the newest `End` in the cycle log, and OSINT's mirror log stops at 2026-08-11 20:10 while CORPUS's mirror has been running the OSINT legs since — 2026-08-14 20:50 in CORPUS's own `logs/mirror_log.md`. So OSINT has been reporting an unmirrored vault, nightly, while the backup ran. **Retire #19 outright — do not repoint it at CORPUS's log** *(Bill, 2026-08-16: all backups are done by CORPUS)*. Repointing was the obvious repair and it is closed off by the boundary: it would make an OSINT lint read a CORPUS file every night, and the one-way rule has exactly one exception, the R9 request feed. A backup OSINT does not perform is not OSINT's to verify.
...
The consequence reaches R10, whose boundary sentence was written with the exception in it and has already landed in OSINT's `CLAUDE.md`. Correcting that is the whole of the superseding file: the boundary loses its exception rather than gaining a workaround. CORPUS still writes `logs/requests-for-osint.csv`, now purely as its own record of what its reports are missing.
...
*(Kept as written, for the record. The list was run in this order on 2026-08-16; only R9's line is corrected below, because following it is what surfaced that the task was impossible.)*
