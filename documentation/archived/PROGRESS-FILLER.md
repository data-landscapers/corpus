...
**Two folders per country: `new-queue\{ISO}\baseline\YYYY-MM-DD-slug.md` and `new-queue\{ISO}\progress\YYYY-MM-DD-slug.md`**, flat within each. Best-effort frontmatter per the source schema (`wiki/schemas.md` §4), with **`places: [{ISO}]`** — a list under a plural key, which is what the schema defines and what this file said wrongly as `place:` until 2026-09-06 (`notes-for-corpus` 17, fifth shape: four of eleven items in one CMR slice arrived with the singular key, and a generator told the wrong name keeps emitting it) — the indicator's Topic L2 slug first in `topics:`, and `sweep_batch: progress-filler-{ISO}-YYYY-MM-DD` — the batch label is what tells OSINT's ingest where these came from.
...
**Added 2026-09-06, from `notes-for-corpus` 17, 18, 19 and 21, which are closed on it.** OSINT drained the batches and reported what it had to correct. None of it reached a page and none of it was owed back; it is recorded here because this is the file a decision to run a second pass would read, and every item below is cheaper to get right at staging than to find at ingest.

**Frontmatter, which is mechanical and should simply be correct.** `date_source` takes `source | proxy | inferred | derived` and nothing else — a batch wrote `stated` 104 times and another wrote `retrieved` six times, both of which mean `proxy`. Filenames must carry no slice suffix: 109 of Algeria's 124 ended `-s1`…`-s6`, and a `raw/` slug is the citation key every wiki page uses, so those would have been permanently misnamed. `entities:` and `ingested:` are required and whole legs shipped without either; `ingested:` is OSINT's stamp and not this run's to write, but `entities:` is derivable from the body that is already in hand. Do not invent keys: `indicator_id:` appeared on several items and is in no schema. And do not copy `schemas.md`'s own example comment into a value — 32 items carried `date_source: source       # source | proxy — …`, harmless to YAML and still the spec leaking into the data.

**Classification is worth less than nothing when it is a guess.** Measured on Cabo Verde, **44 of 78 items had `topics:` corrected at ingest**. `digital.rural` was removed seven times and never once correctly applied; `dpi.govtech`, `gov.legislate`, `dpi.mis` and `finance.new` were each added six or seven times. **`finance.new` is the one that costs something**: a finance record is only built where the item is recognised as finance, so six deals would not have been built at all from the staging alone. Never write a topic slug that is not in `taxonomy.md` — invented values (`econ.fiscal`, `dpi.gov`, `tech.telecom`, `fin.pay`, `gov.service`) and free-text compounds (`gov.legislate--e-commerce-legislation`) are not near-misses, they are outside the vocabulary. **Where the subject is not clear, write nothing rather than a guess**: OSINT reads an absence honestly and re-derives, and that is the note's own ask. `places:`, by contrast, measured fine — one correction in 78 — so the batch-country default is not the systematic error earlier notes described; the real cases are regional and continental instruments, which take `XWA`/`XAF` or the full member list rather than the batch's country.

**A staged `note:` may carry only what its own attached source says.** This is the finding worth the most, and the one no check catches. Three staged notes asserted figures that were true of the subject and absent from the document they were filed with — a health-strategy figure confirmed only from a fuller capture, a mobile-money series confirmed only from the operator's investor sheets, and one locality/coverage series that nothing held supports at all. A slice that verifies the claim finds it stands; only checking it against *that body* fails. The same shape appears as a claim sourced to a page that never discusses the subject — a bilateral-relations history staged to evidence the absence of a hyperscaler agreement, where a page's silence about a topic it never covers is not evidence of anything. **Where a figure comes from elsewhere, name where.** An empty note is free.

**Two capture defects, both since gone, both worth re-checking if capture changes.** Bodies used to stop dead at roughly 51,000 characters, sometimes mid-word, while the frontmatter still said `body_completeness: full` — a truncated body labelled complete is a claim no reader can check without counting characters. The six batches left undelivered on 2026-09-06 carry bodies to 740,000 characters and every capped one is marked `excerpt`, so the ceiling and the mislabelling are both fixed. Separately, one Cabo Verde item contained passages reversed character by character (`odúetnoc od e adaçnava asiuqsep`), which is valid UTF-8, still looks like Portuguese, and passes every structural check there is; nothing of the shape survives in the undelivered batches. A mirrored figure or date read as real would be a wrong fact under a primary-source citation, so if a PDF extractor is ever changed, scan a batch's tails.

**The scope test has two halves and place is only one of them.** The Guinea-Bissau batch dropped 32 of 114 items, 26 of them out of scope outright, against 2 drops in 140 for another country the same day — China debt forgiveness, an EU sanctions decision, a cashew agro-processing loan, a press-conference ban. The country had a military takeover inside the collection window and the batch reads as a sweep of the *country's* news rather than the *subject's*. An item needs an African place **and** a data-governance or digital-transformation subject. **Where a country's in-window digital material is genuinely thin, a thin batch is the right output** — thin coverage is a finding, not a gap to fill with whatever the wires carried that month.
...
**The consumption figures go to Bill in the session, not into a file** *(Bill, 2026-09-01)*. A per-country cost write-up was kept in `documentation/` for the first 27 runs of the series and the 27 files are deleted; the series is past the point where a scale-out estimate wanted a paper trail, and the run's own log line already carries what it found. Report the run in **four stages, each with its own session count and wall time** — sweep, stage-4 read, mapping, render — because they are separated by an OSINT ingest and a mirror refresh and cannot be one sitting. State **yield** as gaps closed against gaps probed, and against sessions spent. The sweep is the visible half and the cheap one; the reading after ingest is a working session on its own, so an estimate counting only the sweep is wrong by roughly the whole of the work.
