...
- The internal wiki compiles — `HUB-COMPILE`, and the hub-`## Financing` prose. **The hubs are private wiki navigation the site never publishes**, so they and the checks that verify them stay OSINT-side. *(Corrected 2026-08-16: this said `REPORT-LINT` B, C, E, F stay and Corpus takes A, D and G–K. Bill's ruling of 2026-08-13 supersedes it — **A–F all stay in OSINT**, next to the records they reconcile, and **G–K travel to Corpus**, because they verify the report layer. `documentation/report-layer.md` and `documentation/archived/osint-migration.md` R3 carry the ruling; this line had not caught up, and R3 points a reader here.)*
- **`outputs/non-state-finance/` and the compile that writes it** *(Bill, 2026-08-16)*. The migration first had OSINT retire this export along with the rest of its `outputs/`, which would have taken `REPORT-LINT` A–E and the hub `## Financing` prose down with it — all of them read `{ISO3}-nonstate.csv`. It is carved out instead: OSINT keeps writing it, no longer as a website feed but as its own compile, and Corpus builds its published copy from the same `raw/` records independently. Two derived copies of one record set, not two authorities. `osint-migration.md` R4 has the detail.
