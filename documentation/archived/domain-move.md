...
**This was built and verified on 2026-08-18.** In the `.io` zone, **Rules → Redirects**, a rule named `force https except acme`: match on the custom expression `not ssl and not starts_with(http.request.uri.path, "/.well-known/acme-challenge/")`, then *URL redirect* → **Dynamic** → `concat("https://", http.host, http.request.uri.path)` → `301`, preserve query string on. **Create the rule before turning the zone setting off**, so there is no window where HTTP is unforced. `http.host` rather than a literal hostname is what makes one rule serve the apex, `www` and `corpus` together.

> **CHECK 40a.** `curl -sS -o NUL -D - http://data-landscapers.io/` must still answer `301` to the `https` form. `curl -sS -o NUL -D - http://data-landscapers.io/.well-known/acme-challenge/test` must answer **`404` and not redirect** — and the response must carry an `X-GitHub-Request-Id` header, which is how you know it reached GitHub rather than being answered at Cloudflare's edge. A `301` on the second means the exclusion is not matching, and renewal will fail in November exactly as issuance failed at step 33.
