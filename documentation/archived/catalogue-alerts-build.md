---
type: design-note
title: catalogue-alerts-build.md — the build of the alerts, and the argument behind it
last_reviewed: 2026-09-20
status: archived 2026-09-20; the build is done and this is not the live reference
---

# The alerts build, archived

**`documentation/catalogue-alerts.md` is the live reference.** It says what an alert is, how the thing is configured, and which rules must not change. This file is what that note carried before the alerts were built and deployed on 2026-09-16: the step-by-step build, the reader-facing copy as it was drafted, the decisions in full with the evidence they were taken on, and the questions the spike closed.

**It is archived rather than deleted because the argument is worth keeping and the procedure is not worth re-reading** *(strategic review 4, R61 — task 37: "the alerts are built and tested, so the design argument goes to `archived/` and the rule stays")*. Every rule still in force was carried up into the live note; what is here is how each was arrived at, and how the account was set up the once.

**Two things below are stale by construction and are kept as they were written.** Part 2's reader-facing text is a snapshot: the live copy is `content/alerts.md`, which has since grown blocks this never had (`already`, `manage-how`, `manage-link`, `manage-confirmed`, `added`, `confirmed`, `e-blocked`) — a copy that diverged, which is the reason the live note now carries a pointer instead. And Part 1's *Later, not in this version* is a backlog, not a record; anything still wanted from it belongs in a job, not here.

`documentation/archived/catalogue-alerts-digest.md` is the design this one displaced.
...
7. Open **Settings → Subscribing → Confirmation** (`https://buttondown.com/settings/subscribing/confirmation`). **It is one block of text** (Bill, 2026-09-16). Paste this into it:
    ...
    ```
    <img src="https://corpus.data-landscapers.io/assets/email-banner.png" width="446" alt="New from Data Landscapers" style="display:block;width:446px;max-width:100%;height:auto;border:0;margin:0 0 16px">
...
8. Open **Settings → Features → Portal** and turn the portal on. It is a **feature**, not a settings page — in the API it is an entry in the newsletter's `enabled_features` list alongside `archives`, `comments` and the rest — which is why *Settings → Portal* was not there to find (Bill, 2026-09-16).
    ...
    **Email design (cosmetics, Bill, 2026-09-16).** Every alert email and the confirmation email open with `site/assets/email-banner.png`: the site logo and *New from Data Landscapers* in the site's green, one image drawn at 3x for 446x80. **It is in the email body because no Buttondown setting can put it at the top of a real email**, which took four rounds to establish: the template puts a 40px `img.newsletter-icon` above the subject and the newsletter name in `h3.colophon` below it; **in a sent email the icon stays 40px whatever CSS says**, so a banner uploaded as the icon arrived squashed; and **the Header field renders only on the web version**. The settings page's preview misleads on all of it — it carries its own rule hiding the header and the colophon, and applies CSS that sent emails do not. So: the newsletter icon (**Settings → General → Branding → Icon**) is `site/assets/email-icon-blank.png`, transparent; the Header toggle is off; **Settings → Email → Custom CSS** is `.newsletter-masthead .subject, .newsletter-masthead .colophon { display: none !important; }`, plus the site's type: body text in `Lato, Calibri, Arial, sans-serif` at 16px in the site's ink `#1a1a18`, and `h3` — which the Worker writes for each catalogue heading — at 17px, weight 600, in the same stack, after `main.css`'s `h3`; and `.email-actions--unsubscribe, .powered-by { text-align: center !important; }`, because the template centres the 600px content table by an HTML attribute mail clients obey but centres the unsubscribe line by CSS they ignore, which left it full-width and left-aligned under a centred box. 600px was kept over full width for line length (Bill) — hiding text *does* survive into sent email, as the vanished name showed; the Worker writes the banner as the digest's first line (`BANNER` in `worker.js`); and A7's confirmation text carries the same `<img>` on its first line. The cost: Buttondown's own hosted pages show no newsletter icon. **Test design changes by sending a draft to yourself, never by the settings preview.**
...
8. **Main-site form** (in `data-landscapers`, its own commit). In `newsletter/index.md`:
   ...
   **Superseded 2026-09-17 (Bill).** The form is gone: `newsletter/index.md` is now a redirect to `/alerts/#site=1`, and the site-wide *Subscribe to news alerts* button points at `/alerts/`. The Corpus form is the only sign-up, so every new subscriber has passed Turnstile.
