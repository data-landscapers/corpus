...
- **The 10 `held` rows need no ingest.** The base already holds a document stating their start position — the selected register names each `raw/` file. **Set 2026-09-11 with the regions' held rows** (below): 8 set, 2 skipped because the held document states no figure for the row (the AfCFTA protocol gives no data-centre count; the AI Strategy gives no committed capital or cost).
...
- **Most held rows are held on their own source.** Where a row's only source predates 2025-09-01 (an enacting act, a 2018 study), that source is its start position, following the XAF precedent.
- **Held rows set, 2026-09-11.** Of the 35 held rows across all six regions, 31 now carry `position_start` and a movement read from the named `raw/` document, and the start source was added to `sources` where missing; the six progress reports were re-rendered. 25 read *No change* (a position fixed before the window and untouched in it), 6 *Advanced*. Two regions rows were skipped because their held document does not state the row's figure: the XWA e-money report counts accounts on a different series from the row's end figures, and the Xalam East Africa page is a PDF cover with no figures.
