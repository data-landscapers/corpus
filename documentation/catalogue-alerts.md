...
3. Open **Settings → Basic**. Confirm the sender name is **Data Landscapers**, and under **Settings → Domains** that the newsletter sends from a `data-landscapers.io` address.

    **Take the root domain and the manual records; decline Buttondown's managed DNS and its Cloudflare integration (2026-09-16).** Buttondown offers two routes. *Managed DNS* delegates a subdomain to Buttondown with two `NS` records, and cannot be used on a root domain at all; *manual* means typing Buttondown's own records into Cloudflare, and works on the root. Two reasons the manual root wins here. A sender on the root reads as `data-landscapers.io` rather than `mail.data-landscapers.io`. And `cloudflare.md` is this project's account of what is in the zone — a delegated subdomain is a part of the zone that account can no longer describe, and an integration holding a DNS-edit grant over it is a write path to the records that serve both sites, the Workers routes and every edition URL, in exchange for saving four rows of typing.

    **A third reason was withdrawn on 2026-09-16, and it leaves a step behind.** This note said the `.io` zone carried no `MX` and no `TXT`, so the root had no SPF record to collide with. `cloudflare.md` said so and was wrong; a resolver shows Microsoft 365 receiving mail and `v=spf1 include:secureserver.net -all` already present. The decision survives it — DKIM is what verification turns on, and it is unaffected — but **if Buttondown's record list includes a `v=spf1` entry, merge its include into the existing row and never add a second.** Two `v=spf1` rows is a PermError that fails authentication for every sender on the domain; one merged row, keeping `-all`, fails nothing. And because that row is a **hard fail** listing only GoDaddy, anything sending as `data-landscapers.io` and absent from it is rejected outright — so the merge decides whether the alerts arrive, not merely whether they authenticate. `cloudflare.md` → *The sending domain* carries the records and the rest of the consequence.

    **Every `CNAME` Buttondown asks for goes in as DNS only — grey cloud.** This is the one that fails silently. `cloudflare.md` records that all three existing hostnames are proxied, so the habit and the dashboard default are both wrong here: a proxied record answers with Cloudflare's addresses instead of Buttondown's, and DKIM verification never passes. `TXT` and `NS` records cannot be proxied, so they need no attention. Add the rows to `cloudflare.md`'s table once they verify, with the grey cloud noted, or the next reader tidying the zone will orange them.
...
5. **`workers/alerts/worker.js`** is a new Worker, separate from `download-log`. `workers/alerts/README.md` is a pointer to this file. Bindings: KV `ALERTS`; secrets `BUTTONDOWN_API_KEY` and `TURNSTILE_SECRET`; variables `SITE` = `https://corpus.data-landscapers.io`, `MAIN_SITE` = `https://data-landscapers.io`, `SEND_MODE` = `draft`. One cron trigger, `0 7 * * 1`.
   ...
   **Alert ids.** The id is the first 10 hex characters of the SHA-256 of the canonical definition, `P=<places sorted, comma-joined, or any>|T=<topics sorted, comma-joined, or any>`. The same selection always gets the same id, so two readers who want *Kenya, Nigeria · AI* share one tag and one section. The tag is named `alert <id>` — 16 characters, inside Buttondown's 100-character limit and its name pattern — with the alert's label as its public description and `subscriber_editable` off.
   ...
   - **The cron handler**, at Monday 07:00 UTC:
     1. Read `last_sent_through`. The window is the day after it to yesterday, capped at 21 days; on the first run, the last 7 days. If `sent:<this Monday>` already exists, stop — a retried cron cannot send twice.
...
10. In the Worker's **Settings → Triggers**, add a cron trigger `0 7 * * 1`.
...
9. With `SEND_MODE` still `draft`, run the cron by hand — the dashboard's **Trigger** button, or `wrangler cron trigger --cron "0 7 * * 1"`.
...
4. **Stop the old main-site alert, once the digest has sent once.** Delete the RSS automation held at Draft in A11 if there was one; if the main-site alert was sent by hand, stop sending it. **Both running is the failure this design exists to prevent** — a reader holding `alert site` would get the same post in a hand-sent mail and again in Monday's digest.
