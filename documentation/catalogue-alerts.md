...
8. Open **Settings → Features → Portal** and turn the portal on. It is a **feature**, not a settings page — in the API it is an entry in the newsletter's `enabled_features` list alongside `archives`, `comments` and the rest — which is why *Settings → Portal* was not there to find (Bill, 2026-09-16).
    ...
    **Email design (cosmetics, Bill, 2026-09-16).** Every email opens with the site icon at 80px and *New from Data Landscapers* beside it in bold green (`#2a6b3a`, the site's `--green`). The icon is `site/assets/logo.png`, uploaded at **Settings → General → Branding → Icon** (a PNG: an SVG does not render in most mail clients); Buttondown serves it at `https://assets.buttondown.email/images/c5bb8087-7a7d-45b2-a003-3afdb261e992.png`. **The template cannot be reordered**, so its own pieces are hidden and redrawn: the template puts a 40px `img.newsletter-icon` above `table.newsletter-masthead`, and the name in that table's `h3.colophon` **below** the subject line. **Settings → Email → Header** holds a two-cell table — the icon at 80px, the name in a styled cell, all styles inline because Gmail and Outlook drop stylesheet CSS — and **Settings → Email → Custom CSS** holds `img.newsletter-icon { display: none !important; }` and `.newsletter-masthead .colophon { display: none !important; }` (without `!important` the template's own rule kept the icon). **The settings page's preview never renders the Header**; a throwaway draft's preview does, and showed icon and name above the subject line.
