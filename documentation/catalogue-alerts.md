...
3. Open **Settings → Basic**. Confirm the sender name is **Data Landscapers**, and under **Settings → Domains** that the newsletter sends from a `data-landscapers.io` address.
    ...
    **Take the root domain and the manual records; decline Buttondown's managed DNS and its Cloudflare integration (2026-09-16).** Buttondown offers two routes. *Managed DNS* delegates a subdomain to Buttondown with two `NS` records, and cannot be used on a root domain at all; *manual* means typing Buttondown's own records into Cloudflare, and works on the root. Three reasons the manual root wins here. The `.io` zone carries **no `MX` and no `TXT`** (`documentation/cloudflare.md` → *Zones and DNS*), so there is no existing SPF record to merge into and none of the usual risk of a second one: the zone is clean, which is exactly the condition under which the root is the safe choice rather than the dangerous one. A sender on the root reads as `data-landscapers.io` rather than `mail.data-landscapers.io`. And `cloudflare.md` is this project's account of what is in the zone — a delegated subdomain is a part of the zone that account can no longer describe, and an integration holding a DNS-edit grant over it is a write path to the records that serve both sites, the Workers routes and every edition URL, in exchange for saving four rows of typing.
