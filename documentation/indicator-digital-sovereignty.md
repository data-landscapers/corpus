...
**Step 1 is therefore a patch to OSINT**, cut per Corpus's `CLAUDE.md`: `scripts/osint-patch.py prepare`, add the row to `lookups/taxonomy.csv` in the clone, `cut`, deliver to `prepared/job-NN/` on the share with the base commit named. If `wiki/taxonomy.md` is outside the set a patch may carry, a `[ACT]` note in `notes-for-osint.md` asks OSINT to add the slug there in the same job, `Affects: lookups/indicators.csv, the maturity assessment's Geopolitics chapter`. Nothing below is minted until the mirror shows the subject.
