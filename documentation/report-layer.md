...
- **The monthly update and the progress report are first issued on the first build after a month closes**, for the month just closed, and are **revised whenever later evidence dated inside their period arrives**. Both carry a period in their filename, and a document a reader can cite by date is not rewritten for the month it is still inside — but once cut it stays open to anything that belongs in it. New sources that move nothing produce **no issue** — the run records `nil, unchanged` and stops. This is what makes the full set affordable: a typical month is ~500 country-tagged sources across ~51 places, a median of 8–10 each, most of which report activity rather than movement.
...
**An issued document's period is fixed at the moment it is cut. Its content is not.** The renderer reads back the `period:` line of a document that already exists, so a re-render never widens a document a reader may already have cited. `--end` overrides, and is for re-cutting a period that was printed wrong.

**No issue is ever closed to new evidence** *(Bill, 2026-08-14)*. A source dated inside a period belongs in that period's document, whenever it arrives — a July-dated item ingested in mid-August is July's news, and if it is of value it is slotted into the July monthly and that document re-cut. The window is defined by `as_at`, the event date, never by the date a source reached the base, which is why the machinery already does this: `moved_in()` selects on `as_at`, so a late arrival falls into the window it belongs to and the re-render picks it up. What the fixed `period:` protects is the *label* — the document does not silently become a report about a longer stretch of time. It says nothing about the content, which improves whenever the evidence does.
