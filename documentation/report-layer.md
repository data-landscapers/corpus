...
**No document may leave the build carrying `_(narrative not yet written)_`.** Where a block has no prose, BUILD.md → *Narrative integrity* governs: remove the section, or write the sentence that explains why there is no suitable narrative. Leaving the marker is not available.
...
**Within a section, every process sub-groups by taxonomy Level-2 subject, in the taxonomy's own order.** Status and progress print one small table per subject, each preceded by a `###` sub-heading carrying that subject's taxonomy label — a section's rows are never combined into one table across subjects. Monthly prints one narrative block per subject, keyed `{section-key}--{subject-slug}` (dots become hyphens: `dpi--dpi-pay`), each under its own `###` sub-heading. **A subject that a section maps but that has no rows — or, for monthly, no rows that moved in the window — gets no sub-heading and no block at all**, in any of the three documents: the drafter is never handed an empty box for a topic with no news. This is a rendering rule, not a drafting one, so it applies to a section however few or many subjects within it actually have content.
