...
| `section` | **Derived, not authoritative** *(2026-08-25)*. A country report's section is the `subject`'s Level-1 chapter in `lookups/taxonomy.csv`, and the renderer computes it rather than reading this column; a region's still comes from `report-region-sections.csv`, by the same subject lookup. The column is kept in step by `normalise_ledger()` so the file reads on its own terms, and it is no longer overridable per row — an override was a way for a row to render under a heading its subject contradicts, which is how `gov.legislate` rows came to sit under *Infrastructure*. |
...
**A country report's sections are the taxonomy's Level-1 chapters and its order is `lookups/taxonomy.csv`'s** *(Bill, 2026-08-25)*. Ten chapters, opening on Governance, the same ten in the same order in all three documents and in the `STATUS-INIT` baseline — and the same order the bulletin has been printing in since it was built. `documentation/report-country-skeleton.md` → *Sections* carries the table and what the change cost. A region is unaffected: `report-region-sections.csv` is a grouping of the region's own objects, not a view of the taxonomy, and it stays.
