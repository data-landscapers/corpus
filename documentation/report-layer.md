...
| `sources` | Source slugs, `\|`-separated, resolved to URLs at render time through `outputs/catalogue/catalogue-internal.csv`. |
...
- **G. Every link is held.** Each URL in a rendered report must be present in `outputs/catalogue/catalogue-internal.csv` — Corpus's own committed catalogue of everything `raw/` holds: the rows a reader can download, plus the `slug` a citation resolves by, which stopped being a published column on 2026-09-09 (`build-catalogue.py` → `CSV_COLS`). `report-render.py` refuses to resolve against a catalogue `raw/` has moved past, counted as well as timed. **Non-optional, and re-run after every edit pass, not once at the end** — a URL synthesised from a remembered pattern is indistinguishable from a real one by inspection; set membership is the only test that catches it.
