...
**Nothing is ever closed to new evidence** *(Bill, 2026-08-14)*. A source belongs in the window its **event date** falls in, whenever it arrives — a July-dated item ingested in mid-August is July's news and goes into the monthly while July is still in its window. `moved_in()` selects on `as_at`, never on when a source reached the base, so a late arrival lands where it belongs. Once a window has slid past a date, evidence dated there still moves the ledger and still shows in the status report and in the progress report's twelve months; the monthly is simply no longer the document that covers it.
...
- **J. As-of honesty.** No document dated ahead of its newest source, and no period comparison made without the shape check recorded.
