...
If something needs to change *in* OSINT — a stale note, a broken path, a request for a new machine-readable export — write it as a numbered finding in `osint-corpus-exchange/notes-for-osint.md` and tell Bill (or whoever owns OSINT now). Don't make the change yourself. This is stated in `CLAUDE.md` and is the single most important rule in this repo.
...
documentation/design.md · osint-corpus-exchange/notes-for-osint.md · CLAUDE.md · documentation/workflow.md
```
...
- **Retention is conditional now** *(Bill, 2026-08-18)*: a superseded edition is deleted unless somebody downloaded it, PDFs and CSVs, forward only — nothing published on or before 2026-08-18 is ever touched. A Cloudflare Worker records which paths are fetched, `scripts/prune-editions.py` acts on it in RENDER Step 6a, and every uncertainty resolves towards keeping the file. `documentation/cloudflare.md` is the reference for the whole layer, and §9 of the design record carries the amendment.
...
These are OSINT-side problems Corpus can't fix directly — they're tracked as unresolved notes 3, 4, 5 in `osint-corpus-exchange/notes-for-osint.md`.

## osint-corpus-exchange/notes-for-osint.md — what it is and why it matters
...
- `documentation/workflow.md` is a lightweight Obsidian kanban board (Backlog / Next / Processing / Complete) — currently near-empty, not a source of truth for project state; treat `documentation/design.md` §6/§7 and `osint-corpus-exchange/notes-for-osint.md` as the actual backlog.
...
3. `osint-corpus-exchange/notes-for-osint.md` — the standing constraints and the 9 open notes.
