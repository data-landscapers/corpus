...
**Prose only, tables excluded: 1,000–1,450 words for a status report, and 300 + 25 to 1,200 + 55 words a row for a monthly** — an intercept plus a rate per ledger row inside the document's own window, resolved against the `ledger_rows` it prints. So a month in which four rows moved is budgeted 400–1,420 words and one in which 158 did is budgeted 4,250–9,890.

**The monthly is the one document whose length is set by its input rather than by its form** *(2026-09-08)*. A flat band was the layer's first answer and it did not survive contact: on 2026-09-08 the 700–2,000 band held 12 of 54 country monthlies, and the twelve were the thin ones. The rest were not overwritten — Nigeria's 7,304 words carried 158 moved rows at 46 words each, which is the floor of a useful block, not padding. A band that reports forty-two documents every night reports nothing: nobody can act on a list that never changes, and a real case of padding would arrive into it invisibly. The per-row form is the same judgement the flat number was trying to express — *a quiet month should still be short* — stated so that it also holds when the month was not quiet.

The floor of a useful block is ~46 words: the fact of a movement *with* the qualification that makes it evidence. The rate is set above that, at 55, because a section carries connective prose the per-row arithmetic does not see; the floor rate of 25 is deliberately half of it, since a document may reasonably fold two related movements into one sentence but cannot fold ten.
