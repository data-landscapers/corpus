...
**The compile timestamp is a claim about the content, not about the build.** The page says when it was last updated, so the stamp moves only when the document moves: `--assemble` rebuilds the document with the stamp already on disk and leaves the file alone if that reproduces it. The stamp has to be excluded from the comparison and everything else has to be in it — a timestamp inside the compared span would make every run look like a change and the gate would never once fire, and anything outside it is a region the generator cannot reach. Both halves of that were learned the hard way the same day; *Which exposed the comparison rule as excluding too much* below is the second.
...
**The same blind spot existed one layer down, in `render.py`** *(found on review, 2026-08-21)*. Its edition gate hashes the body below the frontmatter, for the reason that field is there — everywhere else the frontmatter is machinery and moves on every rewrite. But this document's subtitle *is* its byline, so the gate could not see a byline correction either: `--assemble` would fix the source, the gate would hold the standing edition, and the served page would keep the old wording with nothing reporting it. `render.py`'s `gate_span()` now folds the subtitle into the digest for `type: bulletin` and for no other kind. It cannot cut a spurious edition, because the subtitle's only self-moving field is the stamp and `--assemble` refuses to write a file that differs in nothing else. The cost is one edition, once, when the narrower digest already on the page fails to match.
