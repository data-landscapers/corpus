...
**One bulletin, not two.** There was a country bulletin and a topic bulletin covering the same items over the same window, differing only in grouping — one document rendered under two indexes, and an index is not a document. The country bulletin is retired; what the place grouping was for survives **on the item**, as the place boxes (the website's own `.wip-item-card__status--active` component — shared markup, per the share-style rule).

**The boxes carry a filter** (`site/assets/js/bulletin-filter.js`), the Lab index's category filter adapted for two differences: this list is nested two deep, so a section goes when its last item goes, a category when its last section goes, and the category bar re-punctuates rather than merely hides; and an item has zero to many places (`data-places`, space-separated, written even when empty — an item tagged to no place the site holds a page for is hidden by any selection, which is right). The structure is read off the document at load; the item wrapper `<div class="bulletin-item" data-places="…" markdown="1">` is the only markup the filter needed.

- **A selection shows summaries only** — cross-reference stubs (`bulletin-item--xref`) are dropped, so the count is items, not mentions. The count is checked against the catalogue: for each country, the filter's figure equals the records in `catalogue-internal.csv` carrying that place — the same rows as the published `raw-catalogue.csv`, keyed by the slug the summary store is written under.
- **The filter offers only the places in this edition**, and is omitted below two. Regions come first, in an `<optgroup>` of their own — the catalogue's order in its place facet, and for its reason: *Africa* filed alphabetically between Algeria and Angola is where it is least likely to be looked for. The groups are drawn only when both kinds are present.
- **Regions are boxed and filtered like countries** *(2026-09-04)*. They were not, on the reasoning that an `X__` place had no page to link to — `region.py` builds all eight — so the place dimension was silently dropped for exactly the items that carry nothing else: 12 of the 43 items on 4 September wore no box and left the page the moment the filter was touched, unselectable. **The gate is now the vocabulary, not the leading `X`**: a code gets a box if `place_names()` carries it, which is the same 62 codes the site builds pages for, so a mis-tagged `THA` no longer draws a box onto a 404 either.
- **Matching is exact and there is no roll-up.** Selecting *Africa* gives the items tagged *Africa*, not every item about an African country: the tag is the claim — a continental story is tagged continentally — and a roll-up would make *Africa* a second name for *All*.
...
**`collected_to:` is read from one stated fact, never derived.** OSINT stamps `sweep_closed` — the moment after which nothing more could have been caught — on every path that admits to `raw/`, and `cycle-manifest.json` carries it (`osint_lib.sweep_closed()`). A derivation from heading-cluster gaps was tried and failed in both directions — the constant was a guess about the shape of OSINT's working day, which is not a shape Corpus gets to assume. A stated fact retires a derivation outright rather than joining it as a fallback.

**And it is one fact, not the later of two** *(2026-09-06, `notes-for-corpus` 16)*. The byline compared `sweep_closed` against the sweep cycle's own rotation `End` (`osint_lib.last_cycle_close()`) while the two artefacts were silent about different runs: the nightly cycle stamped no `sweep_closed`, and the two runs that did wrote no rotation row. The manifest is written by every pass that mirrors and states collection itself, so the comparison is retired — `sweep_closed` is collection, which is what the byline claims, and a rotation `End` is a pass finishing, which is a later moment about different work. Where they differ the byline is now the earlier of them, which understates.
