...
- **Alerts for a narrow interest — queued for after the freeze (2026-09-28).** A reader who follows one topic in one country wants to hear when something new matches. **The answer is a filtered feed, not email.** A Worker serves Atom at a URL that holds the filter (`/feed?place=KEN&topic=tech.ai`), reading the published catalogue: `places`, `topics`, `ingested` and `url` are already columns, so this adds no new data and stays metadata-only. The catalogue browser gets a *subscribe to this selection* link that writes that URL for whatever the reader has filtered, so any custom request is covered without a query form. A new edition of a country's reports is a feed item too. The reader's feed reader does the alerting; a reader who wants email puts the URL into an RSS-to-email service, which holds the address instead of us. **Email alerts are ruled out by §1**: an address list is a user record, and it brings consent, unsubscribe and bounce handling with it.

  Two things to settle in the build. **Backfill**: if every new ingest counts as new, a backfill floods subscribers, so items are gated on `published`, or a backfill is capped per update. **Entry IDs** must be stable, and the slug is kept off the site (§2), so an entry's ID is derived from its `url`. About a day's work.
