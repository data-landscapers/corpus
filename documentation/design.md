...
**The bulletin is the one exception, and it proves the rule rather than bending it** *(2026-08-21)*. Hashing below the frontmatter works because the frontmatter is machinery — `compiled:`, `record:` — that moves on every rewrite. The bulletin generates its `subtitle:`, and that subtitle *is* the page's byline: when the document was last updated, and which days it covers. A digest blind to it would hold the standing edition while the served page kept a byline the source had already corrected, and report nothing, because from the outside a held-off edition is what success looks like. So `render.py`'s `gate_span()` folds the subtitle in for `type: bulletin` and for no other kind. It cannot mint a nightly edition — the one field that moves on its own is the stamp, and `bulletin.py --assemble` will not write a file whose sole difference is that — which is the same test the original rule passed, applied to a wider span.
