...
**The `-2` suffix above is not implemented** *(2026-08-18)*. A second content change on one day still overwrites the first day's PDF, which is the failure this section exists to prevent — rare, and rarer now that a run with no change cuts nothing, but real. It is a small change in `render.py` and a matching one in `country.py`, whose `editions()` reads the edition off the last three hyphen-separated parts of a PDF filename and would parse `-2` as part of the date.
