...
- **Alerts for a narrow interest — specified, Bill builds the week of 2026-09-21.** A reader picks up to five countries and five topics, holds up to ten such alerts, and gets **one** email a week carrying a section per alert — including, as one more alert, new writing on data-landscapers.io. A Cloudflare Worker builds the body on a Monday cron and hands it to Buttondown, which holds the addresses, so the site still holds no user record (§1). The implementation steps, the reader text and the decisions are in `documentation/catalogue-alerts.md`.
  ...
  **Buttondown is a list and a sender, and that was the decision.** The design this displaced on 2026-09-16 gave every alert its own Buttondown RSS-to-email feed, which forced one country and one topic per alert, sent a reader with five alerts five Monday emails, and left an alert editable only by dropping it and starting again. Sections in one body cost a KV entry and a tag per combination instead. The price is that every rule about what a reader receives now lives in this repo under its own tests, and a template mistake reaches the whole list at once rather than one feed's subscribers — which is why the first two Mondays send as drafts Bill releases by hand. The displaced design is at `documentation/archived/catalogue-alerts-digest.md` with the comparison.
