...
**`budgets/` is the one source folder, and it is the exception that states the rule** *(strategic review 4 R54, 2026-09-20)*. A budget line read out of a state's own budget document has no record behind it in OSINT's `raw/` — Corpus reads the document and the row *is* the record — so it is authored here, tracked, and regenerated by nothing. `build-finance-page.py` merges it into the budget export and **a country-year present in `budgets/` replaces OSINT's rows for that year rather than adding to them**: the two sides read the same document at different grains, so a union double-counts a programme against its own sub-programmes and no key detects it. `BUDGET-EXTRACT.md` is the procedure, `scripts/budget_source.py` the schema and the checker. The folder is tracked in a public repository, so *Source bodies* below binds it in full: a line's printed name is a label and is carried; everything longer is written in Corpus's own words.
