...
**What the catalogue download carries is a narrower thing than what the catalogue holds** *(Bill, 2026-09-09)*. `raw-catalogue.csv`, the country and region cuts and a reader's filtered selection carry ten columns: title, publisher, author, published, date_precision, places, topics, entities, ingested, url — what identifies a document, places it, and sends a reader to the publisher's own copy. Seven columns came out that day: `slug`, `lens`, `body_completeness`, `finance`, `artefact`, `words` and `url_note`. They are handling notes rather than facts about the document — a key into OSINT's tree, a classification the site no longer shows, a completeness grade for our own copy of a body we do not publish, a filing flag, the names of files only we hold, the length of that unpublished body, and a note written for our own checks about why a record has no URL.

The rows still exist whole on this side: `catalogue-internal.csv` is the download plus `slug` and `url_note` — the key the report layer resolves citations by and the bulletin keys its summary store on, and the note `status_lib` reads to send a citation of a record with no URL to the catalogue entry instead. `raw-catalogue.json` is the full record. Neither is published, linked or copied into `site/`. `build-catalogue.py` → `CSV_COLS` is where the line is drawn, and `catalogue.py` reads it from there rather than restating it, so the browser's cut and the published file cannot disagree.
...
- **Alerts for a narrow interest — built and deployed 2026-09-16; the first digest is Monday 2026-09-21, released by hand.** Sign-up is at `/alerts/` and the catalogue's **Get alerts** button; the Worker is `workers/alerts/worker.js`. A reader picks up to five countries and five topics, holds up to ten such alerts, and gets **one** email a week carrying a section per alert — including, as one more alert, new writing on data-landscapers.io. A Cloudflare Worker builds the body on a Monday cron and hands it to Buttondown, which holds the addresses, so the site still holds no user record (§1). The implementation steps, the reader text and the decisions are in `documentation/catalogue-alerts.md`.

  **Buttondown is a list and a sender, and that was the decision.** The design this displaced on 2026-09-16 gave every alert its own Buttondown RSS-to-email feed, which forced one country and one topic per alert, sent a reader with five alerts five Monday emails, and left an alert editable only by dropping it and starting again. Sections in one body cost a KV entry and a tag per combination instead. The price is that every rule about what a reader receives now lives in this repo under its own tests, and a template mistake reaches the whole list at once rather than one feed's subscribers — which is why the first two Mondays send as drafts Bill releases by hand, and why leaving it on drafts for good is a settled option rather than a delay. The list's existing handful of subscribers, alerted per publication until now, move to the weekly rhythm with it, unannounced (Bill, 2026-09-16) — the same content they subscribed for, up to six days later. The displaced design is at `documentation/archived/catalogue-alerts-digest.md` with the comparison.
...
**`budgets/` is the one source folder, and it is the exception that states the rule** *(strategic review 4 R54, 2026-09-20)*. A budget line read out of a state's own budget document has no record behind it in OSINT's `raw/` — Corpus reads the document and the row *is* the record — so it is authored here, tracked, and regenerated by nothing. `build-finance-page.py` merges it into the budget export and **a country-year present in `budgets/` replaces OSINT's rows for that year rather than adding to them**: the two sides read the same document at different grains, so a union double-counts a programme against its own sub-programmes and no key detects it. `BUDGET-EXTRACT.md` is the procedure, `scripts/budget_source.py` the schema and the checker. **It opened full rather than empty** *(R56a)*: the 488 domestic-state records that built the published table were migrated into it the same day, each row naming the record it came from in `origin_record`, so the Finance page does not depend on a layer that is being retired. A migrated row is held to a narrower bar than one a sitting read, because it carries what its record carried and nothing was invented to fill the difference — and the shortfall is counted per country rather than waived, which is the work order the queue reads. The folder is tracked in a public repository, so *Source bodies* below binds it in full: a line's printed name is a label and is carried; everything longer is written in Corpus's own words.
...
**The pre-worker archive the forward-only clause protected was cleared by hand on 2026-09-08, and the clause now protects nothing the first condition does not.** 1,237 superseded editions published on or before 2026-08-18 — 374 MB, everything from before the download record began — were deleted in one pass by `scripts/drop-pre-worker-editions.py`. The clause exists because *delete unless somebody took it* cannot be applied to a period with no record without deleting everything for want of evidence; that reasoning was sound while the site was public and citations rested on it, and Bill's call is that neither was yet true. **The site is not live and the only fetches in the record are his own testing**, so there was no reader whose citation the exemption was keeping alive — and 374 MB of archive nobody had ever seen was the largest single claim on a 1 GB ceiling. The 164 pre-worker editions that were still the *current* edition of a document unmoved since July were held back, because deleting those would have left live pages offering a 404. **The clause stays in the rule** rather than being struck: it costs nothing now, and re-deriving it would be the harder mistake to notice if the archive is ever repopulated. What is gone is the archive, not the principle — which applies in full to every edition cut from 2026-08-19 onward, and will apply to the live site.
