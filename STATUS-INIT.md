...
5. **One agent for the indicator rows.** All of the country's, from the cut stage 0 wrote, reduced to `Variable Id`, `Value Name`, `Year`, `Comments`, `Source urls`. The comments carry the reasoning and the caveats and are the point; the value code is a summary of them. Same schema.
