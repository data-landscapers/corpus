...
**The one exception is a figure the report computes itself** *(Bill, 2026-08-15: "aggregate doesn't need a link")*. How many commitments there are, what they come to, which subsector took the largest share — no source states these, because they are arithmetic over the evidence rather than claims drawn from it. There is nothing to link, and the rule does not reach them. Until this ruling the finance brief had the extraction agent hang an aggregate on *"the URL of the largest or most representative commitment it rests on"*, which is worse than no link: it puts a hyperlink on a claim the linked source does not establish, and a reader who follows it finds one deal where the sentence promised a total. **An aggregate is written plainly, with no link, in its own paragraph marked `<!-- derived -->`.** The marker is required rather than inferred, because *no link because nothing establishes this* and *no link because the writer forgot* are otherwise the same paragraph. Everything that is not the report's own arithmetic still owes its URL, and the exception is never a way to state a sourced fact without sourcing it.
...
**Known defects in the dataset are recorded once, in `documentation/dpi-data-defects.md`** *(2026-08-20)*, and a run reads them rather than rediscovering them. **One of them is repaired rather than described** *(2026-08-22)*: 92 mojibake sequences across 64 rows and 25 countries, all in `Comments`, where UTF-8 had been read once as cp1252 — the third party's own bytes decoded wrongly, not a claim, so restoring them is not a correction to the data. `python scripts/lint-mojibake.py` says whether it has come back, and matters more than it looks: a mangled cell reaches a baseline as a mangled quote, and before that it silently fails every grep for the word it mangled. In short: the `govtech-*` family — a third of a country's 462 rows — all resolves to one landing page with boilerplate comments and is largely unusable; a recurring set of cells concatenate mutually exclusive answers or repeat an id with conflicting values; and **a sourceless negative is not evidence of absence** — several flat "No"s are contradicted by the wiki outright, so where the dataset is the only thing saying a country lacks an instrument, the finding is ***Not held*** with a `gaps.csv` line. These were confirmed on eight countries before being written down; a ninth run finding them again has found nothing new.
...
**A promoted fact reaches its new sub-section as `mine: true`, and the writer states it in full.** That is the point of promoting it: the section owned nothing, so every fact answering its question belonged to someone else and it could only have alluded to all of them. `costated` and `owner_slug` record where the fact came from; they are provenance and not a restriction. The parent must not tell a writer otherwise — on 2026-08-21 the LBR and SLE runs both briefed the thin chapters that their promoted facts were `mine: false` and were to be referred to in passing only, which is the one thing promotion exists to prevent, and four SLE chapters had to be written twice. The repetition the ownership rule guards against is bounded here by the cap of six, deliberately: a section that gestures at its whole subject is a worse outcome than a figure appearing twice.
...
10. **Add the country's rows to `C:\corpus-osint-xfer\africa-acquire.csv`** — `python scripts/status-acquire.py {ISO3} --compiled {date}`, which asks the catalogue of every cited URL and takes the publisher, title and publication date of an unheld one from the pool. See *Conflicts*. Re-run step 9 afterwards so the frontmatter's `acquire_lines` counts the rows that now exist.

    **Commit anything else standing in the share before the acquire rows, under its own subject line** *(2026-08-21)*. The share is a shared repository and CC commits for both sides, so a run that stages the whole tree sweeps up whatever the other side left there — a `new-queue/` drop, a note edit — and files it under a subject that says this country's baseline. That is the one thing a shared history is kept for, and it went wrong twice before it was noticed, on SLE and again on SOM. Two commits, not one: the other author's work first, named as theirs, then `africa-acquire.csv` alone. Push immediately after each.
...
**One file for all of Africa, not one per country** *(Bill, 2026-08-15)*. The lines accumulate in `C:\corpus-osint-xfer\africa-acquire.csv`, one row per source with the country in a column:
...
This is a queue Bill works down in an OSINT session, and a queue wants to be sorted by publisher, filtered to what is still outstanding, and counted — none of which 54 markdown tables inside 54 report folders will do. `logs/` is where it belongs, beside every other OSINT-ward artefact of this campaign. *(It was `outputs/reports/{ISO3}/{ISO3}-acquire.md` until 2026-08-15, when NGA's was converted and the per-country file retired.)*

**A run rewrites only its own country's rows**, so re-running one country cannot disturb the fifty-three already done. **The `status` and `notes` columns are Bill's and survive every rewrite**, keyed on the country and the URL — the same rule `status-progress.py` follows for its `notes` column, and for the same reason: a file that loses your working state on regeneration is a report, not a worksheet.

**The queue has a far end now, and a closed row does not come back** *(2026-08-24, the first round-trip)*. OSINT's `STATUS-ACQUIRE` takes one country's rows out of `africa-acquire.csv`, queues the sources into its ingest folder, and files the rows in `C:\corpus-osint-xfercquire-done.csv` with a `status` — `staged`, `dropped` with a reason, or `held` — and a `closed` date. A row is in exactly one of the two files, so **both have to be read to know whether a URL owes an acquisition**: `status-acquire.py` skips a closed URL rather than re-queueing work Bill has already done, and `status-check.py` counts closed rows in check FM's `acquire_lines` and treats them as listed for the candidate note. The frontmatter stamps what the baseline *found*; answering a line does not un-find the gap that produced it, and a count that fell to zero the day the queue was worked would put every issued report out of step with its own frontmatter for a reason that is not about the report. AGO was the first country through: 47 rows, 37 staged, 10 dropped.
...
**The counts and the totals in that list are the report's own arithmetic and carry no link** — see *The one hard rule*. They are computed at extraction, where the whole finance cut is in front of one agent, and not by the writer, which sees a slice that has already been filtered to held URLs, deduped and cut by chapter; a count taken there would be a count of surviving facts rather than of commitments. They reach the writer as facts marked `derived`, with an empty `url`, and go on the page in their own `<!-- derived -->` paragraph.
...
- **B — every claim is linked.** No sentence states a fact without a hyperlink on it or on the sentence before it. Two paragraphs are exempt and both are declared rather than inferred: the *not established* sentence, which carries no link by design, and a paragraph marked `<!-- derived -->`, which is a figure the report computed itself and which no source states. The checker reports every derived paragraph it finds, so the exemption stays visible instead of becoming a hole.
...
- **F — every acquire line is dated 2024 or later** and carries date, publisher, title, URL and sub-section. Read from `C:\corpus-osint-xfer\africa-acquire.csv`, filtered to the unit under check.
