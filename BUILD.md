...
5. **Verify** before moving on: `python scripts/report-render.py --unit {ISO3} --check` — checks G (every link held in `index/`), I (vocabulary), **J (no document compiled before the ledger moved)**, **L (no unwritten narrative block)** and **M (every row that states a position cites a source)** must all pass; then the register check `python scripts/report-register-check.py --unit {ISO3}` reports, a human rules.
