...
**A sweep that published nothing into the window has still updated the bulletin** *(Bill, 2026-08-21)*. The stamp moves whenever the material was looked at, not only when something came of it: a run that admits fifty sources of which none is dated inside the two-day window writes the new stamp and reports `checked` rather than `written`. It used to suppress the write on the reasoning that the content had not moved, which left the page saying *last updated the 20th* through a night's work — neglect reported where there was none, and indistinguishable from the real thing.

**Nothing downstream mints an edition over it.** `render.py`'s digest is the body, which the stamp is not in, so the gate holds off through a moved clock — and it refreshes the bulletin's **page** while leaving the dated **PDF** alone (`RENDER.md` → The bulletin). The byline says when we last looked; the PDF is a snapshot and keeps the stamp it was cut with.

**`--assemble` writes nothing when nothing at all has moved**, the clock included. The comparison is the whole file. *(It was the body below the frontmatter until 2026-08-21, which excluded the subtitle along with the stamp — so a correction to the byline ran, reported `unchanged`, and left the wrong wording on disk.)*
...
**The *Last updated* stamp is OSINT's last ingest, read from the mirror** *(Bill, 2026-08-21)* — `logs/ingested_log.md`, via `scripts/osint_lib.py`, a read and nothing more. Every run prints which stamp it used, and `build clock (mirror unreadable)` means the page is stamped with when we ran rather than when the material moved.
