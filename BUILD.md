...
4. **Re-render every document the moved rows fall into**, not just the live one. `--doc status` (regions: `--doc progress`) refreshes current position; a row whose `as_at` falls in an **already-issued** month means that month's documents are now out of date and must be re-cut too: `--doc monthly --month {YYYY-MM}`, and `--doc progress --month {YYYY-MM}` where the window covers it. A July-dated source that arrives in mid-August is July's news — it is slotted into the July monthly, which keeps its period and gains the evidence. `moved_in()` selects on `as_at`, so the window takes it correctly; what it needs is to be told to render.

   Filenames carry no period — `{ISO3}-monthly.md`, `{ISO3}-progress.md` — so `--month` is what decides which issue is being cut, and getting it wrong replaces one issue with another rather than writing a second file. Check the `period:` line after rendering if there is any doubt about which issue a file now holds.

   **Rolling an issue forward is an editing job, and the two documents differ.** A new **monthly** starts with empty blocks — August shares nothing with July — so its narrative is written, not revised. A **progress report** rolls: eleven of its twelve months are the same, so its prose carries across and BUILD **removes what has aged off the back and writes in what has arrived at the front**. A carried sentence describing the month that just dropped out is well-formed and passes every check; finding it is the point of the revision.
