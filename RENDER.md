...
**The download carries eleven columns, and the catalogue holds more than that.** `slug`, `lens`, `body_completeness`, `finance`, `artefact` and `words` came out of `raw-catalogue.csv`, the country cuts and the browser's own selection on 2026-09-09 (`design.md` §2, argued at `build-catalogue.py` → `CSV_COLS`). The page reads the column list from that file by syntax tree, so a cut in a reader's browser and the published file cannot disagree — and `outputs/catalogue/catalogue-internal.csv`, which is the same rows plus the slug, is what the report layer resolves citations through. Neither it nor `raw-catalogue.json` is copied into `site/`.
