...
**Its edition gate sees the byline, which no other document's does** *(2026-08-21)*. `record()` is taken over the body below the frontmatter, because everywhere else the frontmatter is machinery that moves on every rewrite. The bulletin is the exception: its `subtitle:` is generated and is the page's byline — *last updated*, and the days the edition covers — so `render.py`'s `gate_span()` folds it into the digest for `type: bulletin` and for nothing else. Without that, a correction to the byline would land in `outputs/` and stop there, with the standing edition held and the served page still carrying the old wording, and nothing would report it. It cannot cut a spurious edition: the only field that moves on its own is the stamp, and `bulletin.py --assemble` will not write a file whose sole difference is that.
