...
*(Hand this to Claude Code in the Corpus repo. Job 1 turns OSINT's evidence into Corpus-owned `outputs/`. Job 2 (`build/RENDER-RUNBOOK.md`) then renders `outputs/` into the site. Read `MIGRATION-REPORT-LAYER.md` first. OSINT is read-only throughout — never write to it.)*

## The two kinds of work in Job 1

Job 1 has scripted stages and one model-authoring stage, and they are run differently.

The **compiles** (vocab, catalogue, finance, budgets) are pure functions of OSINT's `raw/` and `lookups/`. They are `build/rebuild.py` and they just run.
...
```bash
python build/rebuild.py --all        # vocab snapshot + catalogue + finance/budgets + the scan work order
```

This writes `outputs/catalogue/`, `outputs/non-state-finance/`, `outputs/budgets/`, refreshes `build/vocab/`, and prints the report-update work order (stage 4 below). Commit `outputs/` and `build/vocab/`.
...
The work order (from `--all` above, or `python build/rebuild.py --scan`) lists each initialised unit and how many unconsidered sources it holds. For **each** such unit:

1. **List the new slugs.** `python build/toolchain/report-scan.py --slugs {ISO3}` (run from `build/.workroot/`, which `rebuild.py` sets up). Read each slug's `hub_line`, facets, and body from `raw/` only where the line is not enough.
...
3. **Mark every slug read**, moved or not: `python build/toolchain/report-scan.py --mark {ISO3} <slugs>`. (Sources on `origin_status: hold` are dropped by the script, not marked — pass them in regardless.)
4. **Re-render** the unit's live document: `python build/toolchain/report-render.py --unit {ISO3} --doc status --render` (regions: `--doc progress`).
5. **Verify** before moving on: `python build/toolchain/report-render.py --unit {ISO3} --check` — check G (every link held in `index/`) must pass; then the register check `python build/toolchain/report-register-check.py --unit {ISO3}` reports, a human rules.

Commit the moved ledgers, `considered.txt`, `gaps.csv` and re-rendered docs.

**The Corpus register governs the narrative** (not OSINT's §10): light touch, evidence-led, the lens carried mostly by selection, at most one plain connecting sentence per section, no new figures beyond the ledger. Full statement in `MIGRATION-REPORT-LAYER.md` → *Corpus editorial register*. The evidential spine — tables, dated figures, the *Not held* count — stays exactly as disciplined as OSINT's.

## Stage 5 — re-render (optional, mechanical)

```bash
python build/rebuild.py --reports all      # rebuild every report's tables from its ledger, carry narrative across
```
