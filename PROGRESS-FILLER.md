...
One country per run. Within it, batch the gap indicators **~10–12 per sub-agent, grouped by Level-1** so related briefs are screened and deduped side by side; the parent owns every spawn, no sub-agent spawns another, and each is labelled with its slice (`filler ZAF 2/4 dpi+digital`). Each sub-agent returns a terse tally only — per-indicator staged / dropped / nil — never bodies.
